...
void addSeeds(std::vector<seeding::seed> &fwdmatchingSeeds,
  ...
  for (uint32_t rpos : positions) {
    ...
    thisSeed.pos =
        (thisSeed.reversed) ? readlen - curSeed.pos + k - 2 : curSeed.pos;
...
void createSam(
    ...
    std::unordered_map<size_t,
        ...
        &seedToRefPositions,
    std::string &samFileName, int k, bool pairedEndReads,

    std::vector<char *> &samAlignments, std::string &samHeader) {
  ...
  if (pairedEndReads) {
    ...
    for (int i = 0; i < n_reads / 2; i++) {
      ...
      for (int j = 0; j < seed_counts[i * 2 + 1]; j++) {
        reversed_array[j + seed_counts[i * 2]] =
            readSeeds[i * 2 + 1][j].reversed;
        qry_pos_array[j + seed_counts[i * 2]] = readSeeds[i * 2 + 1][j].pos;
        ref_pos_array[j + seed_counts[i * 2]] =
            readSeeds[i * 2 + 1][j].rpos + k - 1;
      }
  ...
  align_reads(reference, n_reads, read_strings, qual_strings, read_names,
              r_lens, seed_counts, reversed, ref_positions, qry_positions,
              sam_alignments, k, pairedEndReads);
  ...
  if (samFileName.size() > 0) {
    ...
    if (outFile.is_open()) {
      ...
      std::cerr << "Error: failed to write to file " << samFileName
                << std::endl;
    }
  }
  ...
  sort(sam_lines.begin(), sam_lines.end(),
...
void createBam(std::vector<char *> &samAlignments, std::string &samHeader,
               std::string &bamFileName,

               sam_hdr_t *&header, bam1_t **&bamRecords) {
...
void createMplp(std::string &bestMatchSequence, sam_hdr_t *header,
                bam1_t **bamRecords, int numBams, std::string &mpileupFileName,
...
void createMplpBcf(std::string &prefix, std::string &refFileName,
