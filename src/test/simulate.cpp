...
void sim(panmanUtils::Tree* T,
         const std::string& refNode,
         const std::string& out_dir,
         const std::string& prefix,
         const std::string& mut_spec_type,
         const std::vector<double>& num,
         const std::pair<int, int>& indel_len,
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        desc.add_options()("help,h", "Produce help message")(
            ...
            "Reference node name (required). Enter RANDOM to use a random node on the tree, and a new random node is "
            "selected without replacement for each replicate.")(
            ...
            "Number of mutations for snp, insertion, and deletion [10 0 0].")(
            "indel_len",
            po::value<std::vector<int>>()->multitoken(),
            "Min and max indel length [1 9]. Uniform distribution.")(
            "mut_spec",
            po::value<std::string>()->default_value(""),
            "Use input mutation matrix file to model mutations")(
            "mut_spec_type",
            po::value<std::string>()->default_value(""),
            "Type of mutation matrix to use. Options: snp, indel, both. Default: none. Currently only supports snp. "
            "When using snp, the number of mutations and reference nucleotide to mutate are also modeled by the "
            "mutation matrix.")(
            ...
            "model",
            po::value<std::string>()->default_value("NovaSeq"),
            "InSilicoSeq error model [HiSeq]. Options: HiSeq, NextSeq, NovaSeq, MiSeq. For detail, visit InSilicoSeq "
...
std::vector<double> convertMap(const std::unordered_map<int64_t, double>& in) {
    int64_t maxKey = std::max_element(in.begin(),
                                      in.end(),
...
size_t
genLen(const std::pair<int, int>& indel_len, const genotyping::mutationMatrices& mutMat, int type, std::mt19937& gen) {
    if (!mutMat.filled) {
    ...
    switch (type) {
        case 2: probs = convertMap(mutMat.insmat); break;
        case 4: probs = convertMap(mutMat.delmat); break;
        default: break;
    }
...
void genMut(const std::string& curNode,
            ...
            const genotyping::mutationMatrices& mutMat,
            const std::vector<double> mutnum_double,
            const std::pair<int, int> indel_len,
            size_t beg,
            size_t end,
            std::mt19937& gen,
    ...
    for (int i = 0; i < num.size(); i++) {
        for (int j = 0; j < num[i]; j++) {
            switch (i) {
                ...
                case 1: vtp.push_back(2); break;
                case 2: vtp.push_back(4); break;
            }
        }
    }
    ...
    sort(muts.begin(), muts.end(), [](const std::tuple<int, int, int>& a, const std::tuple<int, int, int>& b) {
        ...
        switch (var) {
            ...
            case 2: {
                ...
                nseq = nseq.substr(0, pos + offset + 1) + inss +
                       nseq.substr(pos + offset + 1, nseq.size() - (pos + offset + 1));
            ...
            case 4: {
                ...
                char del = seq[pos];
                nseq =
                    nseq.substr(0, pos + offset + 1) +
                    nseq.substr(pos + offset + get<2>(muts[i]) + 1, nseq.size() - (pos + offset + get<2>(muts[i]) + 1));
                vros << ref << "\t" << del << "\t.\t.\t.\tGT\t1\n";
                offset -= get<2>(muts[i]);
                break;
            }
...
void simReads(const fs::path& fastaOut,
              const fs::path& outReadsObj,
              const std::string& model,
              int n_reads,
              int cpus,
              unsigned seed) {
    std::string cmd = "iss generate --model " + model + " --genomes \'" + fastaOut.string() + "\'" + " -n " +
                      std::to_string(n_reads) + " --output \'" + (outReadsObj / fastaOut.stem()).string() + "\'" +
                      " --cpus " + std::to_string(cpus) + " --seed " + std::to_string(seed);
