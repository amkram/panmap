...
int our_pileup_seq(kstring_t* mplp_string,
    ...
    int del_len = -p->indel;
    if (p->indel > 0) {
        int len = bam_plp_insertion_mod(p, m && !no_ins_mods ? m : NULL, ks, &del_len);
        if (len < 0) {
    ...
    if (del_len > 0) {
        if (no_del < 2) our_printw(-del_len, mplp_string);
        if (!no_del) {
            for (j = 1; j <= del_len; ++j) {
...
void our_print_empty_pileup(kstring_t* k,
            ...
            while (tid > last_tid) {
                if (last_tid >= 0 && !conf->reg) {
                    while (++last_pos < sam_hdr_tid2len(h, last_tid)) {
                        if (conf->bed &&
                            bed_overlap(conf->bed, sam_hdr_tid2name(h, last_tid), last_pos, last_pos + 1) == 0)
                            continue;
                        // TODO handle empty case
                        our_print_empty_pileup(
                            mplp_string, conf, sam_hdr_tid2name(h, last_tid), last_pos, n, ref, ref_len);
                    }
                }
                last_tid++;
                last_pos = -1;
