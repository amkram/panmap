...
std::vector<panmapUtils::NewSyncmerRange> index_single_mode::IndexBuilder::computeNewSyncmerRangesJump(
    ...
    if (localMutationRanges.size() > 1) {
        ...
        for (size_t i = 0; i < localMutationRanges.size(); ++i) {
            sortKeys.emplace_back(
                globalCoords.getScalarFromCoord(localMutationRanges[i].first,
                                                blockStrand[localMutationRanges[i].first.primaryBlockId]),
                i);
        }
    ...
    for (size_t i = 1; i < localMutationRanges.size(); ++i) {
        ...
        // merge if the current and next range are adjacent on their global scalar coordinates
    ...
    while (localMutationRangeIndex < mergedLocalMutationRanges.size()) {
        ...
        // expand left; if it reaches newSyncmerRanges.back(), merge
        ...
        // expand right; if it reaches mergedLocalMutationRanges[localMutationRangeIndex + 1], merge
        offset = 0;
    ...
    for (size_t i = 0; i < newSyncmerRanges.size(); i++) {
        ...
        while (true) {
            ...
            if (curCoordGapMapIt == gapMap.end()) {
                // current and all subsequent positions are non-gap
        ...
        if (i == newSyncmerRanges.size() - 1 && offsetsToDelete != -1) {
            for (size_t j = localRangeSeq.size() - endOffset - offsetsToDelete; j < localRangeSeq.size(); j++) {
                if (refOnSyncmers.contains(localRangeCoordToGlobalScalarCoords[j])) {
                    ...
                    // Only delete seeds inside the genome extent (see flank note above)
...
index_single_mode::IndexBuilder::computeNewKminmerRanges(
    ...
    while (syncmerChangeIndex < static_cast<int64_t>(refOnSyncmersChangeRecord.size())) {
        ...
        int offset = 1;
        if (l == 2) {
            if (curBegIt != state.refOnSyncmersMap.begin() && !newKminmerRanges.empty() &&
                *curBegIt <= *(newKminmerRanges.back().second)) {
                curBegIt = newKminmerRanges.back().first;
                newKminmerRanges.pop_back();
            }
                ...
                if (syncmerChangeIndex != static_cast<int64_t>(refOnSyncmersChangeRecord.size()) - 1) {
                    ...
                    if (nextChangeType != panmapUtils::seedChangeType::DEL) {
                        if (*curEndIt >= nextSyncmerPos) {
                            ...
                            curEndIt = nextSyncmerPos == *state.refOnSyncmersMap.rbegin() ? state.refOnSyncmersMap.end()
                                                                                          : std::next(nextIt);
                            syncmerChangeIndex++;
                            continue;
                        }
...
void index_single_mode::IndexBuilder::buildIndexHelper(panmanUtils::Node* node,
    ...
    panmapUtils::applyMutations(node,
                                dfsIndex,
                                blockSequences,
                                invertedBlocks,
                                globalCoords,
    ...
    // Compute hard mask boundaries, skipping non-gap bases (not scalar positions)
    ...
    std::vector<panmapUtils::NewSyncmerRange> newSyncmerRanges =
        computeNewSyncmerRangesJump(node,
                                    dfsIndex,
                                    blockSequences,
                                    blockExistsDelayed,
                                    blockStrandDelayed,
    ...
    for (const auto& syncmerRange : newSyncmerRanges) {
        ...
        if (effectiveSeq.size() >= indexBuilder.getK()) {
            for (auto [hash, isReverse, isSeed, startPos] : seeding::rollingSyncmers(effectiveSeq,
                                                                                     indexBuilder.getK(),
                                                                                     indexBuilder.getS(),
                ...
                if (!wasSeed && isSeed) {
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        startPosGlobal, panmapUtils::seedChangeType::DEL, refOnSyncmers.at(startPosGlobal));
                    blockOnSyncmersChangeRecord.emplace_back(
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        startPosGlobal, panmapUtils::seedChangeType::SUB, refOnSyncmers.at(startPosGlobal));
                    refOnSyncmers[startPosGlobal] = {hash, endPosGlobal, isReverse};
                }
            }
        }
        ...
        if (oldExists && !newExists) {
            if (blockOnSyncmers.find(blockId) != blockOnSyncmers.end()) {
                ...
                for (uint64_t pos : blockOnSyncmers[blockId]) {
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        pos, panmapUtils::seedChangeType::DEL, refOnSyncmers.at(pos));
                    blockOnSyncmersChangeRecord.emplace_back(blockId, pos, panmapUtils::seedChangeType::DEL);
    ...
    for (size_t i = 0; i < newKminmerRanges.size(); i++) {
        ...
        if (i != 0 && *newKminmerRanges[i - 1].second > beg) {
            output::error("newKminmerRanges[{}].second ({}) > newKminmerRanges[{}].first ({}) in node {}",
                          i - 1,
                          *newKminmerRanges[i - 1].second,
                          i,
                          *newKminmerRanges[i].first,
                          node->identifier);
            std::exit(1);
        }
    }
    ...
    for (size_t i = 0; i < newKminmerRanges.size(); i++) {
        ...
        if (forwardHash != reverseHash) {
            ...
            if (substitution) {
                ...
                refOnKminmersChangeRecord.emplace_back(
                    indexingPos, panmapUtils::seedChangeType::ADD, std::numeric_limits<uint64_t>::max());
        ...
        while (curIt != endIt) {
            ...
            forwardHash = seeding::rol(forwardHash, k) ^ seeding::rol(refOnSyncmers.at(*indexingIt).hash, k * l) ^
                          refOnSyncmers.at(*curIt).hash;
            reverseHash = seeding::ror(reverseHash, k) ^ seeding::ror(refOnSyncmers.at(*indexingIt).hash, k) ^
                          seeding::rol(refOnSyncmers.at(*curIt).hash, k * (l - 1));
            ++indexingIt;
            ...
            if (forwardHash != reverseHash) {
                ...
                if (substitution) {
                    ...
                    refOnKminmersChangeRecord.emplace_back(
                        indexingPos2, panmapUtils::seedChangeType::ADD, std::numeric_limits<uint64_t>::max());
    ...
    // No sort needed: hashes are used directly, not positions.

    // Track seed hash counts for this node (LiteIndex format), starting from parent's counts.
...
void index_single_mode::IndexBuilder::buildIndex() {
    ...
    for (size_t nodeIdx = 0; nodeIdx < numNodes; nodeIdx++) {
        std::sort(nodeChanges[nodeIdx].begin(), nodeChanges[nodeIdx].end(), [](const auto& a, const auto& b) {
            return std::get<0>(a) < std::get<0>(b);
        });
    }
    ...
    for (size_t nodeIdx = 0; nodeIdx < numNodes; nodeIdx++) {
        ...
        for (const auto& [hash, parentCount, childCount] : nodeChanges[nodeIdx]) {
            if (parentCount < INT16_MIN || parentCount > INT16_MAX || childCount < INT16_MIN ||
                childCount > INT16_MAX) {
                output::error(
                    "Seed count overflow at node {}: parentCount={}, childCount={} (Int16 range: [{}, {}]). "
...
void index_single_mode::IndexBuilder::computeSubstitutionSpectrum() {
    ...
    std::function<void(panmanUtils::Node*)> dfs = [&](panmanUtils::Node* node) {
        ...
        if (node != T->root) {
            ...
            for (const auto& nucMutation : node->nucMutation) {
                ...
                bool isSub = (type == panmanUtils::NucMutationType::NS || type == panmanUtils::NucMutationType::NSNPS);
    ...
    // Estimate genome length from sampled leaf nodes
    int64_t genomeLen = 0;
    {
...
void index_single_mode::IndexBuilder::processNode(panmanUtils::Node* node,
                                                  BuildState& state,
                                                  panmapUtils::GlobalCoords& globalCoords,
    ...
    auto& invertedBlocksBacktracks =
        backtrackInfo ? backtrackInfo->invertedBlocksBacktracks : localInvertedBlocksBacktracks;
    auto& refOnSyncmersChangeRecord =
        backtrackInfo ? backtrackInfo->refOnSyncmersChangeRecord : localRefOnSyncmersChangeRecord;
    auto& blockOnSyncmersChangeRecord =
        backtrackInfo ? backtrackInfo->blockOnSyncmersChangeRecord : localBlockOnSyncmersChangeRecord;
    auto& refOnKminmersChangeRecord =
        backtrackInfo ? backtrackInfo->refOnKminmersChangeRecord : localRefOnKminmersChangeRecord;
    auto& gapRunBlockInversionBacktracks =
        backtrackInfo ? backtrackInfo->gapRunBlockInversionBacktracks : localGapRunBlockInversionBacktracks;
    ...
    panmapUtils::applyMutations(node,
                                dfsIndex,
                                state.blockSequences,
                                state.invertedBlocks,
                                globalCoords,
                                localMutationRanges,
                                blockMutationRecord,
                                nucMutationRecord,
                                ...
                                invertedBlocksBacktracks,
                                potentialSyncmerDeletions,
                                state.blockExistsDelayed,
                                state.blockStrandDelayed,
                                imputeAmb_);
    ...
    if (extentGuard_) {
        ...
        auto [newFirstNonGap, newLastNonGap] =
            panmapUtils::computeExtentFromGapMap(state.gapMap, globalCoords.lastScalarCoord, 0);
    ...
    // Seeds in hard-masked regions are ignored (no adds, no deletes)
    const int flankMaskBp = getFlankMaskBp();
    ...
    std::vector<panmapUtils::NewSyncmerRange> newSyncmerRanges =
        computeNewSyncmerRangesJump(node,
                                    ...
                                    blockOnSyncmersChangeRecord,
                                    state.refOnSyncmers,
                                    state.blockOnSyncmers,
                                    state.firstNonGapScalar,
                                    state.lastNonGapScalar);

    // Hard mask: ignore seeds in the first/last flankMaskBp positions.
    for (const auto& syncmerRange : newSyncmerRanges) {
        const auto& [begCoord,
                     endCoord,
                     localRangeSeq,
        ...
        if (effectiveSeq.size() >= static_cast<size_t>(indexBuilder.getK())) {
            for (auto [hash, isReverse, isSeed, startPos] : seeding::rollingSyncmers(effectiveSeq,
                ...
                if (!wasSeed && isSeed) {
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        startPosGlobal, panmapUtils::seedChangeType::DEL, state.refOnSyncmers.at(startPosGlobal));
                    blockOnSyncmersChangeRecord.emplace_back(
                        curBlockId, startPosGlobal, panmapUtils::seedChangeType::DEL);
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        startPosGlobal, panmapUtils::seedChangeType::SUB, state.refOnSyncmers.at(startPosGlobal));
                    state.refOnSyncmers[startPosGlobal] = {hash, endPosGlobal, isReverse};
                }
            }
        }
    ...
    // Handle block mutations that delete entire blocks
    for (const auto& [blockId, oldExists, oldStrand, newExists, newStrand] : blockMutationRecord) {
        if (oldExists && !newExists) {
            if (state.blockOnSyncmers.find(blockId) != state.blockOnSyncmers.end()) {
                ...
                for (uint64_t pos : state.blockOnSyncmers[blockId]) {
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        pos, panmapUtils::seedChangeType::DEL, state.refOnSyncmers.at(pos));
                    blockOnSyncmersChangeRecord.emplace_back(blockId, pos, panmapUtils::seedChangeType::DEL);
                    state.refOnSyncmers.erase(pos);
                    state.refOnSyncmersMap.erase(pos);
                }
    ...
    for (size_t i = 0; i < newKminmerRanges.size(); i++) {
        ...
        while (curIt != endIt) {
            ...
            forwardHash = seeding::rol(forwardHash, k) ^ seeding::rol(state.refOnSyncmers.at(*indexingIt).hash, k * l) ^
                          state.refOnSyncmers.at(*curIt).hash;
            reverseHash = seeding::ror(reverseHash, k) ^ seeding::ror(state.refOnSyncmers.at(*indexingIt).hash, k) ^
                          seeding::rol(state.refOnSyncmers.at(*curIt).hash, k * (l - 1));
            ++indexingIt;
    ...
    // No sorting needed: hashes are used directly, not positions

    // Compute node changes directly from running counts. Skip when walking a path to set up state.
    if (!skipNodeChanges) {
        auto& changes = (*state.nodeChanges)[dfsIndex];

        // Collect all hashes modified at this node
        std::unordered_set<uint64_t> modifiedHashes;
        modifiedHashes.reserve(addedSeedHashes.size() + deletedSeedHashes.size() + substitutedSeedHashes.size() * 2);
        ...
        // Even when skipping nodeChanges, runningCounts must be updated for correct
        // state during subtree traversal
        for (uint64_t hash : addedSeedHashes) {
            state.runningCounts[hash]++;
            runningCountChanges.emplace_back(hash, +1);
        }
    ...
    // Only count nodes where nodeChanges were computed (not path-walk duplicates)
...
void index_single_mode::IndexBuilder::backtrackNode(BuildState& state, const BacktrackInfo& backtrackInfo) {
    ...
    // Revert running count changes in reverse order, applying the opposite delta
    for (auto it = backtrackInfo.runningCountChanges.rbegin(); it != backtrackInfo.runningCountChanges.rend(); ++it) {
...
void index_single_mode::IndexBuilder::buildIndexParallel(int numThreads) {
    ...
    // Divide into N chunks (excluding root, processed separately)
    ...
    arena.execute([&]() {
        tbb::parallel_for(size_t(0), chunks.size(), [&](size_t i) {
            ...
            chunkState.nodeChanges = sharedNodeChanges;
            ...
            std::function<void(panmanUtils::Node*, uint64_t)> processDfsRange = [&](panmanUtils::Node* node,
                ...
                // Compute nodeChanges only if this node is in range
                processNode(node, chunkState, globalCoords, chunkEmptyNodes[i], dfsIdx, &backtrack, !inMyRange);

                // Recurse into children whose subtree may overlap the range
                if (!node->children.empty()) {
                    uint64_t childDfs = dfsIdx + 1;
                    for (auto* child : node->children) {
                        uint64_t childSubtreeEnd = childDfs + subtreeSizes_[child->identifier];
                        // Only recurse if this subtree overlaps the range
            ...
            for (auto* child : T->root->children) {
                uint64_t childSubtreeEnd = childDfs + subtreeSizes_[child->identifier];
                // Only process this subtree if it overlaps the range
                if (childDfs < chunk.endDfs && childSubtreeEnd > chunk.startDfs) {
                    processDfsRange(child, childDfs);
                }
                childDfs = childSubtreeEnd;
    ...
    // nodeChanges were computed during processNode calls
    size_t numNodes = T->allNodes.size();
