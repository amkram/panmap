...
void mgsr::MgsrLiteNode::initializeMutationData(std::vector<std::pair<uint32_t, bool>>& seedDeltas_,
...
void mgsr::MgsrLiteNode::initializeMutationData(std::vector<refSeedChange>& refSeedChanges,
                                                uint32_t curOffset,
                                                uint32_t nextOffset,
                                                size_t numThreads,
                                                bool lowMemory) {
    if (nextOffset > curOffset) {
        this->refSeedChanges = std::vector<refSeedChange>(std::make_move_iterator(refSeedChanges.begin() + curOffset),
                                                          std::make_move_iterator(refSeedChanges.begin() + nextOffset));
    } else if (nextOffset < curOffset) {
        std::cerr << "Error: nextOffset " << nextOffset << " < curOffset " << curOffset
                  << " in mgsr::MgsrLiteNode::initializeMutationData!" << std::endl;
        std::exit(1);
    } else {
...
void mgsr::MgsrLiteNode::removeCollapsedChild(MgsrLiteNode* child) {
    collapsedChildren.erase(std::remove(collapsedChildren.begin(), collapsedChildren.end(), child),
                            collapsedChildren.end());
}
...
void mgsr::MgsrLiteTree::fillTaxonIndices(size_t maximumTaxonNumber) {
    ...
    while (!traversal.empty()) {
        ...
        for (MgsrLiteNode* child : node->children) traversal.push(child);
    }
    ...
    while (!postOrder.empty()) {
        ...
        for (MgsrLiteNode* child : node->children)
            node->taxonIndices.insert(child->taxonIndices.begin(), child->taxonIndices.end());
...
void mgsr::MgsrLiteTree::loadTaxonomicMetadata(const std::string& taxonomicMetadataPath,
    ...
    if (taxonColumn <= 0) {
        std::cerr << "Error: taxonomic rank '" << taxonomicRank << "' not found in header of " << taxonomicMetadataPath
                  << std::endl;
        std::exit(1);
    }
    ...
    while (std::getline(infile, line)) {
        ...
        for (int i = 1; i <= taxonColumn; ++i) {
            if (!(iss >> taxon)) {
                std::cerr << "Error: row for sample '" << sampleId << "' in " << taxonomicMetadataPath
                          << "' has fewer than " << (taxonColumn + 1) << " columns (expected taxon at column "
                          << taxonColumn << ")" << std::endl;
                std::exit(1);
            }
        }
        if (taxon == ".") continue;
...
void mgsr::MgsrLiteTree::initialize(LiteIndex::Reader indexReader,
    ...
    if (seedStartPosReader.size() > 0 || seedEndPosReader.size() > 0) {
        if (seedStartPosReader.size() != seedEndPosReader.size() ||
            seedStartPosReader.size() != seedHashesReader.size()) {
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, seedHashesReader.size(), seedInfos_chunkSize),
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, perNodeChangesReader.size()), [&](const tbb::blocked_range<size_t>& range) {
            for (size_t i = range.begin(); i < range.end(); i++) {
    ...
    auto seedChangeParentCountsReader = indexReader.getSeedChangeParentCounts();
    auto seedChangeChildCountsReader = indexReader.getSeedChangeChildCounts();

    size_t totalSeedChanges = 0;
    for (size_t i = 0; i < seedChangeHashesReader.size(); i++) {
    ...
    for (size_t seg = 0; seg < numSegs; seg++) {
        ...
        size_t segEnd = std::min(segStart + CAPNP_SPLIT, totalSeedChanges);
        auto hashes = seedChangeHashesReader[seg];
        auto parents = seedChangeParentCountsReader[seg];
        auto children = seedChangeChildCountsReader[seg];
        tbb::parallel_for(
            tbb::blocked_range<size_t>(segStart, segEnd, GRAIN_SIZE),
            [&](const tbb::blocked_range<size_t>& range) {
                for (size_t i = range.begin(); i < range.end(); i++) {
                    refSeedChanges[i].hash = hashes[i];
                    refSeedChanges[i].parentCount = parents[i];
                    refSeedChanges[i].currentCount = children[i];
                }
            },
            tbb::simple_partitioner());
    }

    output::trace() << "Successfully read in seed change hashes, parent counts, and child counts, size: "
                    << refSeedChanges.size() << std::endl;
        ...
        if (inserted) {
            if (useSeedInfos) {
                it->second->initializeMutationData(
                    seedDeltas[i], gapRunDeltas[i], invertedBlocks[i], seedInfos, numThreads, lowMemory);
            } else {
                it->second->initializeMutationData(
                    refSeedChanges, nodeChangeOffsets[i], nodeChangeOffsets[i + 1], numThreads, lowMemory);
            }
            if (taxonomicMetadataPath != "") {
    ...
    if (collapseIdenticalNodes) {
        for (auto node : emptyNodesToRemove) {
            ...
            parent->children.erase(std::remove(parent->children.begin(), parent->children.end(), node),
                                   parent->children.end());
            parent->collapsedChildren.erase(
                std::remove(parent->collapsedChildren.begin(), parent->collapsedChildren.end(), node),
                parent->collapsedChildren.end());
...
void mgsr::MgsrLiteTree::calculateRefSeedCountsHelper(MgsrLiteNode* node,
...
void mgsr::MgsrLiteTree::toRefSeedDeltasHelper(MgsrLiteNode* node,
    ...
    for (const auto [seedHash, parSeedCount] : parSeedCounts) {
        ...
        seedCountDeltas.emplace_back(
            seedHash, parSeedCount.first, parSeedCount.second, currentCountFwd, currentCountRev);
    ...
    for (const auto [seedHash, parentCountFwd, parentCountRev, currentCountFwd, currentCountRev] : seedCountDeltas) {
        if (parentCountFwd == 0 && parentCountRev == 0) {
            ...
            refSeedCounts[seedHash] = {parentCountFwd, parentCountRev};
        }
    }
}
...
void mgsr::MgsrLiteTree::setCollapsedDfsIndex(mgsr::MgsrLiteNode* node,
...
void mgsr::MgsrLiteTree::setDfsIndex(mgsr::MgsrLiteNode* node, mgsr::MgsrLiteNode*& prevNode, uint32_t& dfsIndex) {
...
void mgsr::MgsrLiteTree::collapseIdenticalScoringNodes(const absl::flat_hash_set<size_t>& allSeedmerHashesSet) {
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, allNodesVec.size(), 1024), [&](const auto& range) {
        for (size_t i = range.begin(); i != range.end(); ++i) {
            ...
            seedDeltas.erase(std::remove_if(seedDeltas.begin(),
                                            seedDeltas.end(),
                                            [&](const auto& delta) {
                                                return allSeedmerHashesSet.find(seedInfos[delta.first].hash) ==
                                                       allSeedmerHashesSet.end();
                                            }),
                             node->seedDeltas.end());
    ...
    output::trace() << "Tree collapsed... Pruned " << detachedNodes.size() << " nodes, "
                    << allLiteNodes.size() - detachedNodes.size() << " nodes remain." << std::endl;
    output::trace() << "\tPruned " << originalSeedDeltasCount.load() - prunedSeedDeltasCount.load() << " seed deltas, "
...
std::vector<std::tuple<double, mgsr::MgsrLiteNode*, mgsr::MgsrLiteNode*>> mgsr::MgsrLiteTree::getClosestNodesDistance(
    const std::unordered_set<MgsrLiteNode*>& sourceNodes, size_t selectNum, size_t maxPerNode, bool leavesOnly) {
...
applyMutations(panmanUtils::Node* node,
    ...
    for (const auto& nucMutation : node->nucMutation) {
        ...
        if (lastOffset != -1 && blockExists[blockId] && oldBlockExists[blockId] &&
            blockStrand[blockId] == oldBlockStrand[blockId]) {
            if (blockStrand[blockId]) {
                localMutationRanges.emplace_back(panmapUtils::Coordinate(nucMutation, 0),
                                                 panmapUtils::Coordinate(nucMutation, lastOffset));
            } else {
                localMutationRanges.emplace_back(panmapUtils::Coordinate(nucMutation, lastOffset),
                                                 panmapUtils::Coordinate(nucMutation, 0));
            }
        }
    }
...
void mgsr::extractReadSequences(const std::string& readPath1,
        ...
        while (std::getline(file, line)) {
            ...
            while (std::getline(tokenStream, token, '\t')) {
                if (fieldIndex == 0) {
                    ...
                    std::cerr << "Error: Unexpected number of fields in amplicon depth file " << ampliconDepthPath
                              << std::endl;
                }
                ++fieldIndex;
            }
    ...
    if (readPath2.size() > 0) {
        ...
        if (totalReads != forwardReads * 2) {
            std::cerr << "Error: File " << readPath2 << " does not contain the same number of reads as " << readPath1
                      << std::endl;
            std::cerr << "Read sizes: " << forwardReads << " (from " << readPath1 << ") and "
                      << totalReads - forwardReads << " (from " << readPath2 << ")" << std::endl;
            exit(1);
        }
...
void mgsr::ReadDebruijnGraph::searchComponentDFS(RDGNode* node,
                                                 std::unordered_set<RDGNode*>& visited,
                                                 std::vector<RDGNode*>& currentComponent) {
...
void mgsr::ReadDebruijnGraph::sortReads(std::vector<mgsr::Read>& reads,
    ...
    for (const auto& component : connectedComponents) {
        sortReadsHelper(component[0],
                        curReadIndex,
                        reads,
                        sortedReads,
                        readSeedmersDuplicatesIndex,
                        sortedReadSeedmersDuplicatesIndex);
    }
    reads.swap(sortedReads);
    readSeedmersDuplicatesIndex.swap(sortedReadSeedmersDuplicatesIndex);
}

void mgsr::ReadDebruijnGraph::sortReadsHelper(RDGNode* node,
                                              size_t& curReadIndex,
                                              std::vector<mgsr::Read>& reads,
                                              std::vector<mgsr::Read>& sortedReads,
                                              std::vector<std::vector<size_t>>& readSeedmersDuplicatesIndex,
                                              std::vector<std::vector<size_t>>& sortedReadSeedmersDuplicatesIndex) {
    std::stack<RDGNode*> stack;
    std::unordered_set<RDGNode*> visited;
    stack.push(node);
    while (!stack.empty()) {
...
void mgsr::mgsrPlacer::initializeQueryDataBatch(const std::vector<std::string>& readSequences,
                                                std::vector<mgsr::Read>& reads,
    ...
    if (dustThreshold < 100) {
        readDust.resize(readSequences.size());
        for (size_t i = 0; i < readSequences.size(); ++i) {
            readDust[i] = mgsr::getDust(readSequences[i], 64);
        }
    }
    ...
    std::vector<std::pair<std::string_view, std::vector<size_t>>> seqToIndexVec;
    {
        ...
        seqToVecPos.reserve(readSequences.size());
        for (size_t i = 0; i < readSequences.size(); ++i) {
            double curDust = dustThreshold < 100 ? readDust[i] : 0.0;
            if (curDust != 0 && curDust > dustThreshold) continue;
            auto [it, inserted] = seqToVecPos.try_emplace(readSequences[i], seqToIndexVec.size());
            if (inserted) seqToIndexVec.emplace_back(readSequences[i], std::vector<size_t>{});
            seqToIndexVec[it->second].second.push_back(i);
        }
    }

    std::vector<mgsr::Read> uniqueReads(seqToIndexVec.size());
    for (size_t i = 0; i < seqToIndexVec.size(); ++i) {
        ...
        if (forwardRolledHash != reverseRolledHash) {
            ...
            if (refSeedHashSet.count(minHash)) {
                ...
                curRead.seedmersList.emplace_back(mgsr::readSeedmer{minHash,
                                                                    ...
                                                                    (uint32_t)(std::get<3>(syncmers[l - 1]) + k - 1),
                                                                    reverseRolledHash < forwardRolledHash,
                                                                    iorder});
                ++iorder;
            }
            ++curRead.totalSeedmers;
        }

        for (size_t j = 1; j + l <= syncmers.size(); ++j) {
            if (l == 1) {
                if (std::get<1>(syncmers[j])) {
                    reverseRolledHash = std::get<0>(syncmers[j]);
                    forwardRolledHash = std::numeric_limits<size_t>::max();
                } else {
                    forwardRolledHash = std::get<0>(syncmers[j]);
                    reverseRolledHash = std::numeric_limits<size_t>::max();
                }
                if (forwardRolledHash == reverseRolledHash) {
                ...
                const size_t prevHash = std::get<0>(syncmers[j - 1]);
                const size_t nextHash = std::get<0>(syncmers[j + l - 1]);
                forwardRolledHash = seeding::rol(forwardRolledHash, k) ^ seeding::rol(prevHash, k * l) ^ nextHash;
                reverseRolledHash = seeding::ror(reverseRolledHash, k) ^ seeding::ror(prevHash, k) ^
                                    seeding::rol(nextHash, k * (l - 1));
            }

            if (forwardRolledHash == reverseRolledHash) continue;

            size_t minHash = std::min(forwardRolledHash, reverseRolledHash);
            ++curRead.totalSeedmers;

            if (!refSeedHashSet.count(minHash)) continue;

            // try_emplace: single lookup, constructs in-place on insertion
            auto [it, inserted] = curRead.uniqueSeedmers.try_emplace(minHash, std::vector<uint32_t>{});
            if (inserted) {
                it->second.push_back(iorder);
                curRead.seedmersList.emplace_back(
                    mgsr::readSeedmer{minHash,
                                      (uint32_t)std::get<3>(syncmers[j]),
                                      (uint32_t)(std::get<3>(syncmers[j + l - 1]) + k - 1),
                                      reverseRolledHash < forwardRolledHash,
                                      iorder});
            } else {
                it->second.push_back(iorder);
                curRead.seedmersList.emplace_back(
                    mgsr::readSeedmer{it->first,
                                      (uint32_t)std::get<3>(syncmers[j]),
                                      (uint32_t)(std::get<3>(syncmers[j + l - 1]) + k - 1),
                                      reverseRolledHash < forwardRolledHash,
                                      iorder});
        ...
        curRead.discardThreshold = static_cast<int32_t>(discardThreshold * curRead.totalSeedmers);
        if (static_cast<int32_t>(curRead.seedmersList.size()) < curRead.discardThreshold) {
            curRead.seedmersList = {};
            curRead.uniqueSeedmers = {};
        }
    }
    ...
    reads.emplace_back(std::move(uniqueReads[sortedUniqueReadsIndices[0]]));
    readDuplicatesIndex.emplace_back(seqToIndexVec[sortedUniqueReadsIndices[0]].second);
    ...
    for (size_t i = 1; i < sortedUniqueReadsIndices.size(); ++i) {
        ...
        if (currSeedmers.size() == prevSeedmers.size() && currTotalSeedmers == prevTotalSeedmers) {
            ...
            for (size_t j = 0; j < n && isDuplicate; ++j) {
                isDuplicate =
                    (currSeedmers[j].hash == prevSeedmers[j].hash && currSeedmers[j].rev == prevSeedmers[j].rev &&
                     currSeedmers[j].iorder == prevSeedmers[j].iorder);
            }
        }

        if (!isDuplicate) {
            reads.emplace_back(std::move(uniqueReads[sortedIdx]));
            readDuplicatesIndex.emplace_back(std::vector<size_t>{});
        }
        for (const size_t idx : seqToIndexVec[sortedIdx].second) {
            readDuplicatesIndex.back().push_back(idx);
        }
    }

    reads.shrink_to_fit();
    readDuplicatesIndex.shrink_to_fit();

    for (uint32_t i = 0; i < reads.size(); ++i) {
        const auto& sl = reads[i].seedmersList;
        for (uint32_t j = 0; j < sl.size(); ++j) {
            seedmerToReads[sl[j].hash].emplace_back(i, j);
        }
    }
...
void mgsr::ThreadsManager::initializeQueryData(const std::string& readPath1,
                                               const std::string& readPath2,
                                               const std::string& ampliconDepthPath,
                                               double dustThreshold,
                                               uint32_t maskReadsEnds,
                                               bool fast_mode) {
    output::trace() << "Processing query reads... k=" << k << ", s=" << s << ", l=" << l << ", t=" << t
                    << ", openSyncmer=" << openSyncmer << std::endl;

    std::vector<std::vector<std::string>> readSequencesByGroup;
    std::vector<std::vector<std::string>> readNamesByGroup;
    extractReadSequences(
        readPath1, readPath2, ampliconDepthPath, readSequencesByGroup, readNamesByGroup, maskReadsEnds);
    ...
    for (size_t groupId = 0; groupId < readSequencesByGroup.size(); ++groupId) {
                ...
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    ...
                    for (uint64_t i = 1; i < syncmers.size() - l + 1; ++i) {
                        ...
                        if (l == 1) {
                            ...
                            if (forwardRolledHash == reverseRolledHash) {
                                std::cerr << "Error: palindromic syncmers returned during query sketching..."
                                          << std::endl;
                                exit(1);
                            }
                        } else {
                            ...
                            forwardRolledHash = seeding::rol(forwardRolledHash, k) ^
                                                seeding::rol(prevSyncmerHash, k * l) ^ nextSyncmerHash;
                            reverseRolledHash = seeding::ror(reverseRolledHash, k) ^ seeding::ror(prevSyncmerHash, k) ^
                                                seeding::rol(nextSyncmerHash, k * (l - 1));
                        }
                        ...
                        if (forwardRolledHash != reverseRolledHash) {
                            ...
                            if (uniqueSeedmersIt == curRead.uniqueSeedmers.end()) {
                                ...
                                curRead.seedmersList.emplace_back(
                                    mgsr::readSeedmer{minHash,
                                                      (uint32_t)std::get<3>(syncmers[i]),
                                                      (uint32_t)(std::get<3>(syncmers[i + l - 1]) + k - 1),
                                                      reverseRolledHash < forwardRolledHash,
                                                      iorder});
                                ++iorder;
                            } else {
                                ...
                                curRead.seedmersList.emplace_back(
                                    mgsr::readSeedmer{uniqueSeedmersIt->first,
                                                      (uint32_t)std::get<3>(syncmers[i]),
                                                      (uint32_t)(std::get<3>(syncmers[i + l - 1]) + k - 1),
                                                      reverseRolledHash < forwardRolledHash,
                                                      iorder});
        ...
        tbb::parallel_sort(sortedUniqueReadSeedmersIndices.begin(),
                           sortedUniqueReadSeedmersIndices.end(),
                           [&uniqueReadSeedmers, fast_mode](size_t i1, size_t i2) {
        ...
        for (size_t i = 1; i < sortedUniqueReadSeedmersIndices.size(); ++i) {
            ...
            if (currSeedmers.seedmersList.size() == curReads.back().seedmersList.size()) {
                ...
                if (fast_mode) {
                    isDuplicate = std::equal(curr.begin(),
                                             curr.end(),
                                             prev.begin(),
                    ...
                    for (size_t j = 0; j < curr.size() && isDuplicate; ++j) {
                        if (curr[j].hash != prev[j].hash || curr[j].rev != prev[j].rev ||
        ...
        int activeCount =
            (maskReadsRelativeFrequency > 0) + (maskSeedsRelativeFrequency > 0) + (maskReads > 0) + (maskSeeds > 0);
        if (activeCount > 1) {
        ...
        if (maskReadsRelativeFrequency > 0 || maskSeedsRelativeFrequency > 0 || maskReads > 0 || maskSeeds > 0) {
            ...
            size_t maskReadsThreshold =
                (maskReadsRelativeFrequency > 0)
                    ? static_cast<size_t>(maskReadsRelativeFrequency * readSequencesByGroup[groupId].size())
                    : maskReads;

            size_t maskSeedsThreshold =
                (maskSeedsRelativeFrequency > 0)
                ...
                for (size_t readIdx = 0; readIdx < curReads.size(); ++readIdx) {
                    ...
                    if (curRead.seedmersList.size() > 0) {
                        if (writeIdx != readIdx) {
                            ...
                            curReadSeedmersDuplicatesIndex[writeIdx] =
                                std::move(curReadSeedmersDuplicatesIndex[readIdx]);
                        }
                        ++writeIdx;
                    }
                }
    ...
    output::detail() << "Collapsed " << rawReadsSize - numLowComplexityRawReads << " filtered raw reads to "
                     << originalReadsSize << " sketched kminmer sets" << std::endl;
    if (maskReads > 0 || maskReadsRelativeFrequency > 0) {
        output::detail() << "Mask-reads " << (maskReads > 0 ? maskReads : maskReadsRelativeFrequency)
                         << " turned on: " << numMaskReads
                         << " reads containing low coverage kminmers will be skipped during placement and EM... "
                         << "Total reads to process: " << numPassedReads << std::endl;
    }
    if (maskSeeds > 0 || maskSeedsRelativeFrequency > 0) {
        output::detail() << "Mask-seeds " << (maskSeeds > 0 ? maskSeeds : maskSeedsRelativeFrequency)
                         << " turned on: " << numMaskSeeds << " seeds are removed during placement and EM... "
                         << "Total reads to process: " << numPassedReads << std::endl;
    }
    for (size_t i = 0; i < threadsToUse; ++i) {
        output::trace() << "  Thread " << i << " will process " << threadRanges[i].second - threadRanges[i].first
                        << " reads: " << threadRanges[i].first << " -> " << threadRanges[i].second << std::endl;
    }
}
...
void mgsr::updateGapMapStep(std::map<uint64_t, uint64_t>& gapMap,
                            uint64_t start,
                            uint64_t end,
                            bool toGap,
...
void mgsr::invertGapMap(std::map<uint64_t, uint64_t>& gapMap,
    ...
    if (gapMap.empty() ||                           // empty gap map
        ...
        // (leftItExists && start > leftIt->second && rightItExists && start < rightIt->first) || // completely between
        // two gap ranges (leftItExists && start >= leftIt->first && end <= leftIt->second) // completely inside a gap
        // range
    ) {
        return;
    }
...
void mgsr::revertGapMapInversions(std::vector<std::pair<bool, std::pair<uint64_t, uint64_t>>>& gapMapBlocksBacktracks,
                                  std::map<uint64_t, uint64_t>& gapMap) {
...
void mgsr::makeCoordIndex(std::map<uint64_t, uint64_t>& degapCoordIndex,
...
std::vector<panmapUtils::NewSyncmerRange> mgsr::mgsrIndexBuilder::computeNewSyncmerRangesWalk(
    ...
    std::sort(localMutationRanges.begin(),
              ...
              [&globalCoords, &blockStrand](const auto& a, const auto& b) {
                  return globalCoords.getScalarFromCoord(a.first, blockStrand[a.first.primaryBlockId]) <
                         globalCoords.getScalarFromCoord(b.first, blockStrand[b.first.primaryBlockId]);
              });

    std::vector<std::pair<panmapUtils::Coordinate, panmapUtils::Coordinate>> mergedLocalMutationRanges{
        localMutationRanges.front()};
    for (size_t i = 1; i < localMutationRanges.size(); ++i) {
        ...
        if (globalCoords.getScalarFromCoord(curEnd, blockStrand[curEnd.primaryBlockId]) + 1 >=
            ...
            if (globalCoords.getScalarFromCoord(nextEnd, blockStrand[nextEnd.primaryBlockId]) >
                globalCoords.getScalarFromCoord(curEnd, blockStrand[curEnd.primaryBlockId])) {
    ...
    for (size_t i = 0; i < newSyncmerRanges.size(); i++) {
        ...
        while (true) {
            if (!blockExists[curCoord.primaryBlockId]) {
                ...
                curCoord = globalCoords.stepForwardScalar(globalCoords.blockEdgeCoords[curCoord.primaryBlockId].end,
                                                          blockStrand);
                continue;
            }
            ...
            if (curNuc != '-') {
                ...
                blockOnSyncmersChangeRecord.emplace_back(
                    curCoord.primaryBlockId, curScalarCoord, panmapUtils::seedChangeType::DEL);
        ...
        if (i == newSyncmerRanges.size() - 1 && offsetsToDelete != -1) {
            for (size_t j = localRangeSeq.size() - endOffset - offsetsToDelete; j < localRangeSeq.size(); j++) {
                if (refOnSyncmers[localRangeCoordToGlobalScalarCoords[j]].has_value()) {
                    ...
                    blockOnSyncmersChangeRecord.emplace_back(
                        curBlockId, curScalarCoord, panmapUtils::seedChangeType::DEL);
    ...
    for (size_t i = 0; i < newSyncmerRanges.size(); i++) {
        ...
        while (true) {
            ...
            if (curCoordGapMapIt == gapMap.end()) {
                ...
                if (!recomputeInProgress) {
                    if (curCoord != curBegCoord) {
                        ...
                        curCoordScalar =
                            globalCoords.getScalarFromCoord(curCoord, blockStrand[curCoord.primaryBlockId]);
                    }
            ...
            if (curNuc != '-') {
                ...
                if (blockOnSyncmers[curCoord.primaryBlockId].empty()) blockOnSyncmers.erase(curCoord.primaryBlockId);
                blockOnSyncmersChangeRecord.emplace_back(
                    curCoord.primaryBlockId, curCoordScalar, panmapUtils::seedChangeType::DEL);
        ...
        if (i == newSyncmerRanges.size() - 1 && offsetsToDelete != -1) {
            for (size_t j = localRangeSeq.size() - endOffset - offsetsToDelete; j < localRangeSeq.size(); j++) {
                if (refOnSyncmers[localRangeCoordToGlobalScalarCoords[j]].has_value()) {
                    ...
                    if (blockOnSyncmers[curBlockId].empty()) blockOnSyncmers.erase(curBlockId);
                    blockOnSyncmersChangeRecord.emplace_back(
                        curBlockId, curGlobalScalarCoord, panmapUtils::seedChangeType::DEL);
                    seedsToDelete.push_back(localRangeCoordToGlobalScalarCoords[j]);
                }
            }
        }
...
mgsr::mgsrIndexBuilder::computeNewKminmerRanges(
        ...
        for (const auto& [syncmerPos, changeType, rsyncmer] : refOnSyncmersChangeRecord) {
            if (changeType == panmapUtils::seedChangeType::DEL) {
                ...
                if (it == refOnSyncmersMap.end()) {
                    std::cerr << "Error: syncmer position not found in refOnSyncmersMap when computing new k-min-mer "
                                 "ranges.\n";
                    exit(1);
                }
    ...
    while (syncmerChangeIndex < refOnSyncmersChangeRecord.size()) {
        ...
        int offset = 1;
        if (l == 2) {
            if (curBegIt != refOnSyncmersMap.begin() && !newKminmerRanges.empty() &&
                *curBegIt <= *(newKminmerRanges.back().second)) {
                ...
                if (syncmerChangeIndex != refOnSyncmersChangeRecord.size() - 1) {
                    ...
                    if (nextChangeType != panmapUtils::seedChangeType::DEL) {
                        if (*curEndIt >= nextSyncmerPos) {
                            ...
                            curEndIt = nextSyncmerPos == *refOnSyncmersMap.rbegin() ? refOnSyncmersMap.end()
                                                                                    : std::next(nextIt);
                            syncmerChangeIndex++;
                            continue;
                        }
...
void mgsr::mgsrIndexBuilder::buildIndexHelper(panmanUtils::Node* node,
    ...
    std::sort(gapRunUpdates.begin(), gapRunUpdates.end(), [&](const auto& a, const auto& b) {
    ...
    // // not really needed for building the index... But do need to keep it for debugging and comparing with brute
    // force std::map<uint64_t, uint64_t> degapCoordIndex; std::map<uint64_t, uint64_t> regapCoordIndex; if (dfsIndex >=
    // 0) {
    ...
    std::vector<panmapUtils::NewSyncmerRange> newSyncmerRanges;
    if (useJump) {
        newSyncmerRanges = computeNewSyncmerRangesJump(node,
                                                       dfsIndex,
                                                       blockSequences,
                                                       blockExistsDelayed,
                                                       blockStrandDelayed,
                                                       ...
                                                       gapMap,
                                                       localMutationRanges,
                                                       blockOnSyncmersChangeRecord);
    } else {
        newSyncmerRanges = computeNewSyncmerRangesWalk(node,
                                                       dfsIndex,
                                                       blockSequences,
                                                       blockExistsDelayed,
                                                       blockStrandDelayed,
    ...
    for (const auto& syncmerRange : newSyncmerRanges) {
        const auto& [begCoord,
                     endCoord,
                     localRangeSeq,
                     localRangeCoordToGlobalScalarCoords,
                     localRangeCoordToBlockId,
                     seedsToDelete] = syncmerRange;
        if (localRangeSeq.size() >= indexBuilder.getK()) {
            for (auto [hash, isReverse, isSeed, startPos] : seeding::rollingSyncmers(localRangeSeq,
                                                                                     indexBuilder.getK(),
                                                                                     indexBuilder.getS(),
                                                                                     indexBuilder.getOpen(),
                                                                                     indexBuilder.getT(),
                                                                                     true)) {
                ...
                if (!wasSeed && isSeed) {
                    ...
                    blockOnSyncmersChangeRecord.emplace_back(
                        curBlockId, startPosGlobal, panmapUtils::seedChangeType::ADD);
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        startPosGlobal, panmapUtils::seedChangeType::DEL, refOnSyncmers[startPosGlobal].value());
                    blockOnSyncmersChangeRecord.emplace_back(
                        curBlockId, startPosGlobal, panmapUtils::seedChangeType::DEL);
                    ...
                    refOnSyncmersChangeRecord.emplace_back(
                        startPosGlobal, panmapUtils::seedChangeType::SUB, refOnSyncmers[startPosGlobal].value());
                    refOnSyncmers[startPosGlobal] = {hash, endPosGlobal, isReverse};
                }
            }
        }
    ...
    for (size_t i = 0; i < newKminmerRanges.size(); i++) {
        ...
        if (i != 0 && *newKminmerRanges[i - 1].second > beg) {
            std::cerr << "Error: newKminmerRanges[" << i - 1 << "].second (" << *newKminmerRanges[i - 1].second
                      << ") > newKminmerRanges[" << i << "].first (" << *newKminmerRanges[i].first << ") in node "
    ...
    for (size_t i = 0; i < newKminmerRanges.size(); i++) {
        ...
        if (forwardHash != reverseHash) {
            seeding::uniqueKminmer_t uniqueKminmer{*indexingIt,
                                                   refOnSyncmers[*curIt].value().endPos,
                                                   std::min(forwardHash, reverseHash),
                                                   reverseHash < forwardHash};
            bool substitution = curRefOnKminmer.has_value();
            if (substitution) {
                refOnKminmersChangeRecord.emplace_back(
                    *indexingIt, panmapUtils::seedChangeType::SUB, curRefOnKminmer.value());
            } else {
                refOnKminmersChangeRecord.emplace_back(
                    *indexingIt, panmapUtils::seedChangeType::ADD, std::numeric_limits<uint64_t>::max());
            }
            ...
            if (curRefOnKminmer.has_value()) {
                refOnKminmersChangeRecord.emplace_back(
                    *indexingIt, panmapUtils::seedChangeType::DEL, curRefOnKminmer.value());
        ...
        while (curIt != endIt) {
            ...
            forwardHash = seeding::rol(forwardHash, k) ^ seeding::rol(refOnSyncmers[*indexingIt].value().hash, k * l) ^
                          refOnSyncmers[*curIt].value().hash;
            reverseHash = seeding::ror(reverseHash, k) ^ seeding::ror(refOnSyncmers[*indexingIt].value().hash, k) ^
                          seeding::rol(refOnSyncmers[*curIt].value().hash, k * (l - 1));
            ++indexingIt;
    ...
    if (!newKminmerRanges.empty() && newKminmerRanges.back().second == refOnSyncmersMap.end()) {
        ...
        for (size_t j = 0; j < l - 1; j++) {
            ...
            if (refOnKminmers[*delIt].has_value()) {
                refOnKminmersChangeRecord.emplace_back(
                    *delIt, panmapUtils::seedChangeType::DEL, refOnKminmers[*delIt].value());
    ...
    for (const auto& [syncmerPos, changeType, rsyncmer] : refOnSyncmersChangeRecord) {
        if (changeType == panmapUtils::seedChangeType::DEL && refOnKminmers[syncmerPos].has_value()) {
            refOnKminmersChangeRecord.emplace_back(
                syncmerPos, panmapUtils::seedChangeType::DEL, refOnKminmers[syncmerPos].value());
    ...
    if (indexFull) {
        ...
        std::sort(deletedSeedIndices.begin(), deletedSeedIndices.end(), [&](const auto& a, const auto& b) {
            return uniqueKminmers[a].startPos < uniqueKminmers[b].startPos;
        });
        while (deletedIdx < deletedSeedIndices.size() && addedIdx < addedSeedIndices.size() &&
               substitutedIdx < substitutedSeedIndices.size()) {
...
void mgsr::mgsrIndexBuilder::buildIndex() {
    ...
    capnp::List<uint32_t>::Builder seedStartPosBuilder =
        indexBuilder.initSeedStartPos(indexFull ? uniqueKminmers.size() : 0);
    capnp::List<uint32_t>::Builder seedEndPosBuilder =
        indexBuilder.initSeedEndPos(indexFull ? uniqueKminmers.size() : 0);
...
void mgsr::mgsrPlacer::addSeedAtPosition(uint64_t newKminmerIndex) {
    ...
    delayedRefSeedmerStatus[hash] = positions.size() == 1 ? mgsr::RefSeedmerExistStatus::EXIST_UNIQUE
                                                          : mgsr::RefSeedmerExistStatus::EXIST_DUPLICATE;
}
...
void mgsr::mgsrPlacer::subSeedAtPosition(uint64_t newKminmerIndex) {
    ...
    if (oldHashToPositionIt->second.size() == 1) {
        ...
        auto eraseIt = std::find(oldHashToPositionIt->second.begin(), oldHashToPositionIt->second.end(), posMapIt);
        oldHashToPositionIt->second.erase(eraseIt);
        delayedRefSeedmerStatus[oldHash] = oldHashToPositionIt->second.size() == 1
                                               ? mgsr::RefSeedmerExistStatus::EXIST_UNIQUE
                                               : mgsr::RefSeedmerExistStatus::EXIST_DUPLICATE;
    }
    oldKminmerIndex = newKminmerIndex;
    auto& positions = hashToPositionMap[newHash];
    positions.push_back(posMapIt);
    delayedRefSeedmerStatus[newHash] = positions.size() == 1 ? mgsr::RefSeedmerExistStatus::EXIST_UNIQUE
                                                             : mgsr::RefSeedmerExistStatus::EXIST_DUPLICATE;
}
...
void mgsr::mgsrPlacer::delSeedAtPosition(uint64_t kminmerIndex) {
    ...
    if (hashToPositionIt->second.size() == 1) {
        ...
        auto eraseIt = std::find(hashToPositionIt->second.begin(), hashToPositionIt->second.end(), posMapIt);
        hashToPositionIt->second.erase(eraseIt);
        delayedRefSeedmerStatus[hash] = hashToPositionIt->second.size() == 1
                                            ? mgsr::RefSeedmerExistStatus::EXIST_UNIQUE
                                            : mgsr::RefSeedmerExistStatus::EXIST_DUPLICATE;
    }
    positionMap.erase(posMapIt);
...
mgsr::RefSeedmerExistStatus mgsr::mgsrPlacer::getCurrentRefSeedmerExistStatus(uint64_t hash) {
    ...
    std::cerr << "Error: getRefSeedmerExistStatus: hash " << hash << " exists in " << hashToPositionIt->second.size()
              << " positions" << std::endl;
    std::exit(1);
    return mgsr::RefSeedmerExistStatus::NOT_EXIST;
}
...
void mgsr::mgsrPlacer::updateSeeds(MgsrLiteNode* node, std::unordered_set<uint64_t>& affectedSeedmers) {
    const auto& curSeedDeltas = node->seedDeltas;
    ...
    for (size_t i = 0; i < curSeedDeltas.size(); i++) {
        ...
        if (i != curSeedDeltas.size() - 1 &&
            seedInfos[seedIndex].startPos == seedInfos[curSeedDeltas[i + 1].first].startPos) {
...
void mgsr::squareEM::updateProps1() {
    ...
    for (size_t i = 0; i < numNodes; ++i) {
        props1(i) = (readDuplicates.array() * (probs.col(i).array() * props0[i] * inverseDenoms.array())).sum() *
                    invTotalWeight;
    }
}
...
void mgsr::squareEM::updateProps2() {
    ...
    inverseDenoms.noalias() = denoms.cwiseInverse();
    for (size_t i = 0; i < numNodes; ++i) {
        props2(i) = (readDuplicates.array() * (probs.col(i).array() * props1[i] * inverseDenoms.array())).sum() *
                    invTotalWeight;
    }
}
...
void mgsr::squareEM::updateProps(const Eigen::VectorXd& original, Eigen::VectorXd& result) {
    ...
    inverseDenoms.noalias() = denoms.cwiseInverse();
    for (size_t i = 0; i < numNodes; ++i) {
        result(i) = (readDuplicates.array() * (probs.col(i).array() * original[i] * inverseDenoms.array())).sum() *
                    invTotalWeight;
    }
}
    ...
    while (curIteration < maximumIterations) {
        ...
        if (maxChangeThreshold == 0) {
            if (output::config().verbose)
                std::cout << "\rEM iteration " << curIteration << " difference: " << std::fixed << std::setprecision(8)
            ...
            if (output::config().verbose)
                std::cout << "\rEM iteration " << curIteration << " max_change: " << std::fixed << std::setprecision(8)
                          << max_change << std::flush;
...
void mgsr::mgsrPlacer::updateRefSeedmerStatus(size_t hash,
...
void mgsr::mgsrPlacer::updateSeedmerChangesTypeFlag(mgsr::RefSeedmerChangeType seedmerChangeType,
...
uint64_t mgsr::mgsrPlacer::extendMinichain(std::map<uint64_t, uint64_t>::const_iterator refPositionIt,
                                           const mgsr::Read& curRead,
                                           uint64_t& curEnd,
                                           bool rev,
                                           uint64_t qidx,
                                           uint64_t c) {
...
void mgsr::mgsrPlacer::extendChainRemoval(uint64_t& c,
                                          uint64_t& curEnd,
                                          const std::vector<mgsr::affectedSeedmerInfo>& affectedSeedmerInfos,
                                          uint32_t lastSeedmerIndex) {
...
void mgsr::mgsrPlacer::extendChainAddition(uint64_t& c,
                                           uint64_t& curEnd,
                                           const std::vector<mgsr::affectedSeedmerInfo>& affectedSeedmerInfos,
                                           bool chainRev,
...
void mgsr::mgsrPlacer::extendChainUpdate(uint64_t& c,
                                         uint64_t& curEnd,
                                         const std::vector<mgsr::affectedSeedmerInfo>& affectedSeedmerInfos,
                                         bool chainRev,
                                         std::map<uint64_t, uint64_t>::const_iterator refPositionIt,
...
bool mgsr::mgsrPlacer::colinearAdjacent(std::map<uint64_t, uint64_t>::const_iterator fromIterator,
                                        std::map<uint64_t, uint64_t>::const_iterator toIterator,
                                        bool fromRev,
                                        bool toRev) {
    ...
    if (curMinichains.empty()) {
        ...
        if (addRangeEnd == originalMinichainBeg - 1 && originalMinichainBeg != 0) {
            ...
            if (addRangeRev != originalMinichainRev) {
                ...
                // same direction
                auto originalRangeBegKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[originalMinichainBeg].hash)->second.front();
                auto newRangeEndKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[addRangeEnd].hash)->second.front();
            ...
            if (addRangeRev != originalMinichainRev) {
                ...
                // same direction
                auto originalRangeEndKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[originalMinichainEnd].hash)->second.front();
                auto newRangeBegKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[addRangeBeg].hash)->second.front();
                if (colinearAdjacent(
                        originalRangeEndKminmerPositionIt, newRangeBegKminmerPositionIt, originalMinichainRev)) {
        ...
        auto rightMinichainIt =
            std::upper_bound(curMinichains.begin(), curMinichains.end(), minichain, mgsr::compareMinichainByBeg);
        ...
        if (leftMinichainExists) {
            ...
            if (addRangeRev == leftMinichainRev && leftMinichainEnd + 1 == addRangeBeg) {
                auto newRangeBegKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[addRangeBeg].hash)->second.front();
                auto leftRangeEndKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[leftMinichainEnd].hash)->second.front();
        ...
        if (rightMinichainExists) {
            ...
            if (addRangeRev == rightMinichainRev && addRangeEnd + 1 == rightMinichainBeg) {
                auto newRangeEndKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[addRangeEnd].hash)->second.front();
                auto rightRangeBegKminmerPositionIt =
                    hashToPositionMap.find(curSeedmerList[rightMinichainBeg].hash)->second.front();
...
void mgsr::mgsrPlacer::removeFromMinichains(std::vector<mgsr::Minichain>& curMinichains, mgsr::Minichain minichain) {
    ...
    if (curMinichains.size() == 1) {
        ...
        auto currentOriginalMinichainIt =
            std::upper_bound(curMinichains.begin(), curMinichains.end(), minichain, mgsr::compareMinichainByBeg);
        ...
        if (removeRangeEnd > currentOriginalMinichainEnd) {
            ...
            if (currentOriginalMinichainBeg == removeRangeBeg) {
                ...
                currentOriginalMinichainIt->endIndex = removeRangeBeg - 1;
                curMinichains.insert(currentOriginalMinichainIt + 1,
                                     {(uint32_t)(removeRangeEnd + 1),
                                      (uint32_t)currentOriginalMinichainEnd,
                                      currentOriginalMinichainRev});
            }
        }
    }
}

void mgsr::mgsrPlacer::updateMinichainsMixed(size_t readIndex,
                                             const std::vector<mgsr::affectedSeedmerInfo>& affectedSeedmerInfos) {
    mgsr::Read& curRead = reads[readIndex];
    const auto& curSeedmerList = curRead.seedmersList;
    auto& curMinichains = curRead.minichains;
...
void mgsr::mgsrPlacer::updateMinichains(size_t readIndex,
                                        const std::vector<mgsr::affectedSeedmerInfo>& affectedSeedmerInfos,
                                        bool allUniqueToNonUnique,
                                        bool allNonUniqueToUnique) {
    mgsr::Read& curRead = reads[readIndex];
    const auto& curSeedmerList = curRead.seedmersList;
    auto& curMinichains = curRead.minichains;
...
void mgsr::mgsrPlacer::fillReadToAffectedSeedmerIndex(
    ...
    for (const auto& hash : affectedSeedmers) {
        ...
        for (const auto& [readIndex, affectedSeedmerIndex] : affectedseedmerToReads->second) {
            ...
            auto [affectedSeedmerIndexVectorIt, inserted] = readToAffectedSeedmerIndex.try_emplace(
                readIndex, std::vector<mgsr::affectedSeedmerInfo>{}, std::pair<bool, bool>{});
...
bool mgsr::mgsrPlacer::isColinearFromMinichains(mgsr::Read& curRead,
    ...
    if (!minichainRev) {
        absl::flat_hash_map<size_t, mgsr::hashCoordInfoCache>::iterator rbeg1CoordInfoCacheIt =
            hashCoordInfoCacheTable.find(beg1seedmer.hash);
        absl::flat_hash_map<size_t, mgsr::hashCoordInfoCache>::iterator rbeg2CoordInfoCacheIt =
            hashCoordInfoCacheTable.find(beg2seedmer.hash);
        absl::flat_hash_map<size_t, mgsr::hashCoordInfoCache>::iterator rend1CoordInfoCacheIt =
            hashCoordInfoCacheTable.find(end1seedmer.hash);
        ...
        absl::flat_hash_map<size_t, mgsr::hashCoordInfoCache>::iterator rbeg1CoordInfoCacheIt =
            hashCoordInfoCacheTable.find(end1seedmer.hash);
        absl::flat_hash_map<size_t, mgsr::hashCoordInfoCache>::iterator rbeg2CoordInfoCacheIt =
            hashCoordInfoCacheTable.find(end2seedmer.hash);
        absl::flat_hash_map<size_t, mgsr::hashCoordInfoCache>::iterator rend2CoordInfoCacheIt =
            hashCoordInfoCacheTable.find(beg2seedmer.hash);
...
void mgsr::mgsrPlacer::computeOverlapCoefficientsHelper(
    ...
    if (curDfsIndex % 1000 == 0) {
        if (output::config().verbose)
            std::cout << "\r" << curDfsIndex << " / " << liteTree->allLiteNodes.size() - liteTree->detachedNodes.size()
                      << std::flush;
    }
    ...
    auto updateKminmerCount = [&](size_t seedHash, bool seedRev, bool isInsertion) -> bool {
        if (isInsertion) {
            auto [it, inserted] =
                seedRev ? kminmerOnRefCount.try_emplace(seedHash, 0, 1) : kminmerOnRefCount.try_emplace(seedHash, 1, 0);
    ...
    for (const auto& [seedIndex, toDelete] : curSeedDeltas) {
        ...
        if (toDelete) {
            ...
            if (allSeedmerHashesSet.find(seedHash) != allSeedmerHashesSet.end() && shouldUpdate)
    ...
    overlapCoefficients.emplace_back(node->identifier,
                                     static_cast<double>(binaryOverlapKminmerCount) /
...
mgsr::mgsrPlacer::computeOverlapCoefficients(const absl::flat_hash_set<size_t>& allSeedmerHashesSet) {
...
void mgsr::mgsrPlacer::placeReadsHelper(MgsrLiteNode* node) {
    ...
    if (threadId == 0) {
        kminmerOverlapCoefficients[node->identifier] =
            static_cast<double>(binaryOverlapKminmerCount) / static_cast<double>(hashToPositionMap.size());
    }
    ...
    if (affectedSeedmers.empty()) {
        ...
        for (auto& [readIndex, affectedSeedmerIndexInfo] : readToAffectedSeedmerIndex) {
            ...
            //   std::cerr << "[" << node->identifier << " " << curDfsIndex << "] readIndex: " << readIndex << "
            //   dynamic: " << pseudoScore << " != brute force: " << bruteForceScore << std::endl; exit(1);
            // }

            // if (curRead.duplicates.size() > excludeDuplicatesThreshold * curRead.seedmersList.size())
            // readTypes[readIndex] = mgsr::readType::HIGH_DUPLICATES;
        }
    }
...
void mgsr::mgsrPlacer::scoreNodesHelper(MgsrLiteNode* node,
    ...
    for (auto child : node->collapsedChildren) {
        scoreNodesHelper(child,
                         processingNode,
                         numDuplicates,
                         readWEPPWeights,
                         readScores,
                         curNodeSumRawScore,
                         curNodeSumEPPRawScore,
                         curNodeSumWEPPScore);
    }
        ...
        if (score == curRead.maxScore) {
            ...
            if (lastParsimoniousNode != nullptr) {
                if (lastParsimoniousNode == processingNode->nextNodeDfsCollapsed) {
                    ...
                    curRead.eppNodeRanges.emplace_back(
                        lastParsimoniousNode, processingNode->nextNodeDfsCollapsed, false);
                    lastParsimoniousNode = nullptr;
                }
            }
        }
...
void mgsr::mgsrPlacer::scoreNodes(const std::unordered_map<size_t, uint32_t>& seedNodesFrequency) {
    ...
    for (size_t i = 0; i < reads.size(); ++i) {
        ...
        numDuplicates[i] =
            threadsManager->readSeedmersDuplicatesIndex[threadsManager->threadRanges[threadId].first + i].size();
        // readWEPPWeights[i] = numDuplicates / static_cast<double>((curRead.seedmersList.size() - curRead.maxScore + 1)
        // * pow(curRead.epp, 2));
        if (seedNodesFrequency.empty()) {
            readWEPPWeights[i] =
    ...
    scoreNodesHelper(liteTree->root,
                     processingNode,
                     numDuplicates,
                     readWEPPWeights,
                     readScores,
...
void mgsr::mgsrPlacer::assignReadsHelper(mgsr::MgsrLiteNode* node,
...
void mgsr::mgsrPlacer::assignReadsBatchHelper(
    ...
    for (auto child : node->collapsedChildren) {
        assignReadsBatchHelper(child, assignedReadsByNode, mpsReadSet);
    }
...
void mgsr::mgsrPlacer::checkTaxonIndicesBatch(MgsrLiteNode* node,
                                              size_t maximumTaxonNumber,
    ...
    for (const auto [readIndex, scoreDelta] : curNodeScoreDeltas) {
        ...
        int curAmbiguousScoreThreshold =
            std::max(ambiguousScoreThreshold, static_cast<int>(curMaxScore * ambiguousScoreThresholdRatio));

        if (scoreDelta == curMaxScore ||
            scoreDelta >= std::max(0, static_cast<int>(curMaxScore) - curAmbiguousScoreThreshold)) {
            bool exceeded = node->overMaximumTaxonNumber;
            if (!exceeded) {
                curRead.taxonIndices.insert(node->taxonIndices.begin(), node->taxonIndices.end());
                exceeded = curRead.taxonIndices.size() > maximumTaxonNumber;
...
void mgsr::mgsrPlacer::assignReadsBatch(std::unordered_map<MgsrLiteNode*, std::vector<size_t>>& assignedReadsByNode,
...
void mgsr::mgsrPlacer::calculateBreadthRatio(
    ...
    double observedBreadthRatio =
        totalRefSeeds > 0 ? static_cast<double>(observedBreadthCount) / static_cast<double>(totalRefSeeds) : 0.0;
    ...
    double observedToExpectedBreadthRatio =
        expectedBreadthRatio > 0 ? observedBreadthRatio / expectedBreadthRatio : 0.0;

    breaths.emplace_back(node,
                         mgsr::breadthInfo{totalRefSeeds,
                                           observedBreadthCount,
                                           observedBreadthRatio,
                                           totalDepth,
                                           meanDepth,
                                           expectedBreadthRatio,
                                           observedToExpectedBreadthRatio});
...
void mgsr::ThreadsManager::scoreNodesMultithreaded() {
    ...
    auto duration_scoreNodes =
        std::chrono::duration_cast<std::chrono::milliseconds>(end_time_scoreNodes - start_time_scoreNodes);
    ...
    for (size_t i = 0; i < reads.size(); ++i) {
        const auto& curRead = reads[i];
        if (curRead.maxScore == 0) continue;
        // readWEPPWeights[i] = static_cast<double>(readSeedmersDuplicatesIndex[i].size()) /
        // static_cast<double>((curRead.seedmersList.size() - curRead.maxScore + 1) * pow(curRead.epp, 2));
        if (seedNodesFrequency.empty()) {
            readWEPPWeights[i] =
    ...
    for (auto& [_, node] : liteTree->allLiteNodes) {
        ...
        const size_t curNodeSumRawScore =
            std::accumulate(node->sumRawScoresByThread.begin(), node->sumRawScoresByThread.end(), 0);
        const size_t curNodeSumEPPRawScore =
            std::accumulate(node->sumEPPRawScoresByThread.begin(), node->sumEPPRawScoresByThread.end(), 0);
    ...
    while (true) {
        ...
        output::trace() << "inserted " << topWEPPNode->identifier << " into selected nodes. WEPP score: " << std::fixed
                        << std::setprecision(5) << topWEPPNode->sumWEPPScore
                        << "... corrected: " << topWEPPNode->sumWEPPScoreCorrected
                        << ", active reads remaining: " << activeReads.size()
                        << ", raw reading remaining: " << rawReadRemaining << std::endl;
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, threadRangesForReadsToSubtract.size()),
                          [&](const tbb::blocked_range<size_t>& rangeIndex) {
                              for (size_t i = rangeIndex.begin(); i != rangeIndex.end(); ++i) {
                                      ...
                                      for (const auto& eppRange : curRead.eppNodeRanges) {
                                          ...
                                          while (true) {
                                              ...
                                              if (selectedNodes.find(curNode) == selectedNodes.end()) {
                                                  nodeScoreChangesByThread[i][curNode].subtract(
                                                      readWEPPWeights[readIndex]);
                                              }
        ...
        for (auto& [_, node] : liteTree->allLiteNodes) {
            ...
            if (node->identifier == liteTree->debugNodeID) {
                output::trace() << "Debug node " << node->identifier
                                << " corrected WEPP score: " << node->sumWEPPScoreCorrected << std::endl;
            }
    ...
    auto duration_selectNodes =
        std::chrono::duration_cast<std::chrono::milliseconds>(end_time_selectNodes - start_time_selectNodes);
...
void mgsr::ThreadsManager::countSeedNodesFrequencyHelper(mgsr::MgsrLiteNode* node,
    ...
    for (const auto& [seedIndex, toDelete] : curSeedDeltas) {
        ...
        if (toDelete) {
            ...
            if (kminmerOnRefCountIt == kminmerOnRefCount.end()) {
                std::cerr << "Error: trying to delete a seedHash that does not exist in kminmerOnRefCount!"
                          << std::endl;
                exit(1);
            }
            ...
            if (kminmerOnRefCountIt->second == 0) {
                ...
                if (lastMatchNode != nullptr) {
                    if (lastMatchNode == processingNode->nextNodeDfsCollapsed) {
                        ...
                        seedNodesFrequency[seedHash] +=
                            processingNode->nextNodeDfsCollapsed->collapsedDfsIndex - lastMatchNode->collapsedDfsIndex;
...
void mgsr::ThreadsManager::computeNodeSeedScoresHelper(MgsrLiteNode* node,
                                                       std::unordered_map<size_t, int32_t>& kminmerOnRefCount,
    ...
    ++curDFSIndex;
    for (MgsrLiteNode* child : node->collapsedChildren) {
        computeNodeSeedScoresHelper(
            child, kminmerOnRefCount, selectedNodes, seedWeights, curNodeSeedScore, curDFSIndex);
    }
...
void mgsr::ThreadsManager::computeNodeSeedScores() {
    ...
    const auto& seedInfos = liteTree->seedInfos;
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, nodeVec.size(), 1024), [&nodeVec, &seedInfos, &seedWeights](const auto& range) {
            for (size_t i = range.begin(); i != range.end(); ++i) {
                ...
                seedDeltas.erase(std::remove_if(seedDeltas.begin(),
                                                seedDeltas.end(),
                                                [&](const auto& delta) {
                                                    return seedWeights.find(seedInfos[delta.first].hash) ==
                                                           seedWeights.end();
                                                }),
                                 seedDeltas.end());
            }
    ...
    std::unordered_map<size_t, int32_t> kminmerOnRefCount;
    computeNodeSeedScoresHelper(
        liteTree->root, kminmerOnRefCount, selectedNodes, seedWeights, curNodeSeedScore, curDFSIndex);
    ...
    while (true) {
        ...
        output::trace() << "Selected node " << maxCorrectedScoreNode->identifier << " with corrected seed score "
                        << maxCorrectedScore << ", was (" << nodeSeedScores[maxCorrectedScoreNode]
                        << "), selected nodes so far: " << selectedNodes.size()
                        << ", remaining seeds: " << seedWeights.size() << std::endl;
        ...
        tbb::parallel_for(
            tbb::blocked_range<size_t>(0, threadRangesForSeedsToSubtract.size()),
            [&](const tbb::blocked_range<size_t>& rangeIndex) {
                for (size_t i = rangeIndex.begin(); i != rangeIndex.end(); ++i) {
                    ...
                    for (size_t j = start; j < end; ++j) {
                        ...
                        if (curSeedNodeMatchRangesIt == seedMatchedNodeRanges.end()) {
                            std::cerr
                                << "Error: seed hash not found in seedMatchedNodeRanges during computeNodeSeedScores!"
                                << std::endl;
                        }
...
void mgsr::mgsrPlacer::scoreReadsHelper(mgsr::MgsrLiteNode* node, mgsr::MgsrLiteNode*& processingNode) {
    if (progressBar && progressTracker) {
        progressTracker->incrementProgress(threadId);
    }
    ...
    auto updateReadsForSeed = [&](size_t seedHash, bool seedRev, int scoreDelta) {
        ...
        for (const auto& pair : seedmerToReadsIt->second) {
            ...
            modifiedReads.try_emplace(readIndex,
                                      mgsr::ModifiedReadInfo{read.numForwardMatching, read.numReverseMatching});
    ...
    if (!modifiedReads.empty()) {
        if (lowMemory) {
            ...
            currentNodeScoreDeltasGrouped.emplace_back(
                mgsr::readScoreDeltaLowMemory{.readIndex = readIndex, .scoreDelta = (int16_t)newScore});
            auto currentGroup = &currentNodeScoreDeltasGrouped.back();
        ...
        if (newScore == curRead.maxScore) {
            ...
            if (lastParsimoniousNode != nullptr) {
                if (lastParsimoniousNode == processingNode->nextNodeDfsCollapsed) {
                    ...
                    curRead.epp += processingNode->nextNodeDfsCollapsed->collapsedDfsIndex -
                                   lastParsimoniousNode->collapsedDfsIndex;
                    lastParsimoniousNode = nullptr;
                }
            }
        } else {
...
void mgsr::mgsrPlacer::scoreReadsBatchHelper(mgsr::MgsrLiteNode* node,
                                             mgsr::MgsrLiteNode*& processingNode,
    ...
    for (const auto [seedHash, parentCountFwd, parentCountRev, currentCountFwd, currentCountRev] :
        ...
        if (parentCountFwd == 0 && currentCountFwd > 0) {
            updateReadsForSeed(seedHash, false, 1);
        } else if (parentCountFwd > 0 && currentCountFwd == 0) {
            updateReadsForSeed(seedHash, false, -1);
        }
        if (parentCountRev == 0 && currentCountRev > 0) {
            updateReadsForSeed(seedHash, true, 1);
        } else if (parentCountRev > 0 && currentCountRev == 0) {
            updateReadsForSeed(seedHash, true, -1);
        }
    }
    ...
    if (!modifiedReads.empty()) {
        ...
        if (!currentNodeScoreDeltas.empty()) {
            std::cerr << "Error: currentNodeScoreDeltas is not empty" << std::endl;
            exit(1);
        }
        currentNodeScoreDeltas.reserve(totalReadsToKeep);
        for (const auto& [readIndex, _] : modifiedReads) {
            auto& curRead = reads[readIndex];
            if (curRead.readType != mgsr::ReadType::PASS) continue;
            ...
            if (curRead.keepForFilterAndAssign) {
                currentNodeScoreDeltas.emplace_back(readIndex, newScore);
            }
    ...
    for (MgsrLiteNode* child : node->collapsedChildren) {
        scoreReadsBatchHelper(child, processingNode, discard_threshold);
    }
...
void mgsr::mgsrPlacer::scoreReadsBatch(double discard_threshold) {
    MgsrLiteNode* processingNode = nullptr;
...
void mgsr::ThreadsManager::computeKminmerCoverageHelper(
        ...
        if (lowMemory) {
            ...
            for (const auto& scoreDelta : curNodeThreadScoreDeltasLowMemory) {
                ...
                if (readScoresBacktrackIndex == readScoresBacktrack.size()) {
                    readScoresBacktrack.emplace_back(threadReadStart + readIndex,
                                                     readScores[threadReadStart + readIndex]);
                } else {
                    ...
                    if (curDecodedTrailingDelta > -8) {
                        if (readScoresBacktrackIndex == readScoresBacktrack.size()) {
                            readScoresBacktrack.emplace_back(threadReadStart + readIndex + i + 1,
                                                             readScores[threadReadStart + readIndex + i + 1]);
                        } else {
                            readScoresBacktrack[readScoresBacktrackIndex].first = threadReadStart + readIndex + i + 1;
                            readScoresBacktrack[readScoresBacktrackIndex].second =
                                readScores[threadReadStart + readIndex + i + 1];
                        }
                        readScoresBacktrackIndex++;
            ...
            for (const auto& scoreDelta : curNodeThreadScoreDeltas) {
                if (readScoresBacktrackIndex == readScoresBacktrack.size()) {
                    readScoresBacktrack.emplace_back(threadReadStart + scoreDelta.readIndex,
                                                     readScores[threadReadStart + scoreDelta.readIndex]);
                } else {
        ...
        if (oldScore != maxScore && newScore == maxScore) {
            for (const auto& [seedmer, _] : reads[readIndex].uniqueSeedmers) {
                ...
                auto [coveredKminmerIt, coveredKminmerInserted] =
                    coveredKminmers.try_emplace(seedmer, std::unordered_set<uint32_t>({readIndex}));
    ...
    //   std::cout << "Covered kminmers size mismatch: brute force " << coveredKminmersBruteForce.size() << " != dynamic
    //   " << coveredKminmers.size() << std::endl; exit(1);
    // }
    ...
    //     std::cout << "Covered kminmer " << seedmer << " reads size mismatch: brute force " << reads.size() << " !=
    //     dynamic " << coveredKminmerIt->second.size() << std::endl; exit(1);
    //   }
    //   for (const auto& readIndex : reads) {
    //     if (coveredKminmerIt->second.find(readIndex) == coveredKminmerIt->second.end()) {
    //       std::cout << "Covered kminmer " << seedmer << " read index " << readIndex << " not found in
    //       coveredKminmers" << std::endl; exit(1);
    //     }
    //   }
    // }

    kminmerCoverage[node->identifier] =
        static_cast<double>(coveredKminmers.size()) / static_cast<double>(refKminmers.size());
    ...
    for (size_t i = 0; i < curSeedDeltas.size(); i++) {
        ...
        if (i != curSeedDeltas.size() - 1 &&
            ...
            auto delRefKminmerIt = refKminmers.find(delSeed.hash);
            if (delRefKminmerIt == refKminmers.end()) {
                std::cerr << "Error: backtracking seed " << curSeedDeltas[i + 1].first << " not found in refKminmers"
                          << std::endl;
                exit(1);
            }
            delRefKminmerIt->second--;
    ...
    for (size_t i = 0; i < coveredKminmersBacktrackIndex; ++i) {
        ...
        if (toErase) {
            ...
            if (coveredKminmerIt == coveredKminmers.end()) {
                std::cerr << "Error: backtracking coveredKminmer " << hash << " not found in coveredKminmers"
                          << std::endl;
                exit(1);
            }
            coveredKminmerIt->second.erase(readIndex);
            ...
            auto [coveredKminmerIt, coveredKminmerInserted] =
                coveredKminmers.try_emplace(hash, std::unordered_set<uint32_t>({readIndex}));
            if (!coveredKminmerInserted) coveredKminmerIt->second.insert(readIndex);
        }
    }
}
...
std::vector<uint32_t> mgsr::ThreadsManager::getScoresAtNode(const std::string& nodeId,
...
void mgsr::ThreadsManager::getScoresAtNode(const std::string& nodeId,
                                           std::vector<uint32_t>& curNodeScores,
                                           const std::vector<size_t>& readIndexOffset) const {
    MgsrLiteNode* currentNode = liteTree->allLiteNodes[nodeId];
                    ...
                    for (size_t i = 0; i < numTrailing; ++i) {
                        ...
                        if (curDecodedTrailingDelta > -8) {
                            ...
                            curNodeScores[readIndexOffset[threadReadStart + readIndex + i + 1]] +=
                                currentScoreDelta + curDecodedTrailingDelta;
                        }
                    }
                }
            } else {
...
void mgsr::ThreadsManager::printStats() {
    ...
    for (size_t i = 0; i < threadRanges.size(); ++i) {
        output::trace() << i << "\t" << threadRanges[i].second - threadRanges[i].first << "\t"
                        << readMinichainsInitialized[i] << "\t" << readMinichainsAdded[i] << "\t"
                        << readMinichainsRemoved[i] << "\t" << readMinichainsUpdated[i] << std::endl;
    }
}

mgsr::squareEM::squareEM(mgsr::ThreadsManager& threadsManager,
    ...
    if (overlapCoefficientCutoff > 0) {
        ...
        std::sort(nodesVec.begin(), nodesVec.end(), [](const auto& a, const auto& b) { return a->ocRank < b->ocRank; });
        ...
        output::detail() << "Overlap coefficients selected " << probableNodes.size()
                         << " nodes, emLeavesOnly: " << emLeavesOnly << std::endl;
    ...
    std::vector<std::pair<std::vector<uint32_t>, std::vector<std::string_view>>> scoresToNodeIdsVector(
        scoresToNodeIds.size());
...
std::vector<mgsr::MgsrLiteNode*>
mgsr::getNearestNodes(mgsr::MgsrLiteNode* node, uint32_t numNodes, bool leafNodesOnly) {
    ...
    while (!pq.empty() && result.size() < numNodes) {
        ...
        if (current.node->parent) {
            ...
            if (distances.find(current.node->parent) == distances.end() || newDist < distances[current.node->parent]) {
        ...
        for (mgsr::MgsrLiteNode* child : current.node->children) {
            ...
            if (distances.find(child) == distances.end() || newDist < distances[child]) {
...
std::vector<mgsr::MgsrLiteNode*> mgsr::getNearestNodes(mgsr::MgsrLiteNode* node,
