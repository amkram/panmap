...
void noFilter(
    std::vector<std::string> &nodes, Eigen::MatrixXd &probs,
    const std::unordered_map<std::string,
                             tbb::concurrent_vector<std::pair<int32_t, double>>>
        &allScores,
    const std::unordered_map<std::string, std::string>
        &leastRecentIdenticalAncestors,
    const std::vector<bool> &lowScoreReads, const size_t &numLowScoreReads,
    const std::string &excludeNode, const std::vector<bool> &excludeReads) {
  std::cerr << "No preEM filtering" << std::endl;

  size_t numExcludedReads =
      std::count(excludeReads.begin(), excludeReads.end(), true);

  std::cerr << "Excluding " << numExcludedReads
            << " reads with high number of duplicates" << std::endl;

  if (!excludeNode.empty()) {
    probs.resize(allScores.begin()->second.size() - numExcludedReads,
                 allScores.size() - leastRecentIdenticalAncestors.size() - 1);
  } else {
    probs.resize(allScores.begin()->second.size() - numExcludedReads,
                 allScores.size() - leastRecentIdenticalAncestors.size());
  }
  size_t colIndex = 0;

  for (const auto &node : allScores) {
    if (leastRecentIdenticalAncestors.find(node.first) !=
        leastRecentIdenticalAncestors.end())
      continue;
    if (!excludeNode.empty() && node.first == excludeNode)
      continue;
...
get_nth_order_neighbors(Tree *T, const std::string &node, int n_order) {
  ...
  while (!bfs_queue.empty()) {
    ...
    if (level <= n_order) {
      ...
      if (T->allNodes[current_node]->parent &&
          visited.find(T->allNodes[current_node]->parent->identifier) ==
              visited.end()) {
        bfs_queue.push(
            {T->allNodes[current_node]->parent->identifier, level + 1});
...
void filter_method_1(
    ...
    const std::string &excludeNode, const std::vector<bool> &excludeReads,
    Tree *T, int n_order) {
  std::cerr << "Filter method 1: filter out haplotypes that do not have a "
               "unique best read score"
            << std::endl;
  ...
  std::vector<std::vector<int32_t>> scoresMatrix(
      allScores.begin()->second.size() - numExcludedReads);
  for (auto &vec : scoresMatrix)
    vec.resize(allScores.size() - leastRecentIdenticalAncestors.size());

  std::vector<std::string> allNodes(allScores.size() -
                                    leastRecentIdenticalAncestors.size());
  size_t colIndex = 0;
  for (const auto &node : allScores) {
    if (leastRecentIdenticalAncestors.find(node.first) !=
        leastRecentIdenticalAncestors.end())
  ...
  for (size_t i = 0; i < scoresMatrix.size(); ++i) {
    ...
    if (highestScoringNode != "")
      filteredNodes.insert(highestScoringNode);
  }
  ...
  for (const auto &filteredNode : filteredNodes) {
    ...
    for (const auto &node : identicalSet) {
      std::unordered_set<std::string> nth_order_neighbors =
          get_nth_order_neighbors(T, node, n_order);
      for (const auto &neighbor : nth_order_neighbors) {
        std::string neighbor_leastRecentIdenticalAncestor = neighbor;
        if (leastRecentIdenticalAncestors.find(neighbor) !=
            leastRecentIdenticalAncestors.end()) {
          neighbor_leastRecentIdenticalAncestor =
              leastRecentIdenticalAncestors.at(neighbor);
        }
        if (nodes_seen.find(neighbor_leastRecentIdenticalAncestor) ==
                nodes_seen.end() &&
            leastRecentIdenticalAncestors.find(
                neighbor_leastRecentIdenticalAncestor) ==
                leastRecentIdenticalAncestors.end()) {
  ...
  std::cout << "Finished filter method 1: " << nodes.size()
            << " nodes.. prob matrix size: " << probs.rows() << " x "
            << probs.cols() << std::endl;
}
...
void filter_method_2(
    ...
    const std::vector<bool> &lowScoreReads, const size_t &numLowScoreReads,
    const std::string &excludeNode, const std::vector<bool> &excludeReads,
    size_t minHighestScoreCount) {
  std::cerr << "Filter method 2: filter out haplotypes that do not have a best "
               "read score"
            << std::endl;
  ...
  std::cerr << "Finished filter method 2: " << nodes.size() << " nodes"
            << std::endl;
}
...
void filter_method_3(
    ...
    const std::string &excludeNode, const std::vector<bool> &excludeReads,
    Tree *T, int n_order) {
  std::cerr << "Filter method 3: select nodes that are local optima and their "
  ...
  std::cerr << "Finished filter method 3: " << nodes.size() << " nodes"
            << std::endl;
}
