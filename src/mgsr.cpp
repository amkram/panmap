...
void mgsr::mgsrIndexBuilder::buildIndexHelper(
  ...
  panmapUtils::applyMutations(node, dfsIndex, blockSequences, invertedBlocks, globalCoords, localMutationRanges, blockMutationRecord, nucMutationRecord, gapRunUpdates, invertedBlocksBacktracks, potentialSyncmerDeletions, blockExistsDelayed, blockStrandDelayed, imputeAmb);
...
void mgsr::mgsrPlacer::updateGapMap(
  ...
  for (const auto [startPos, endPos, toGap] : curGapRunDeltas) {
    gap_map::updateGapMapStep(gapMap, startPos, endPos, toGap, gapMapBacktracks, dummyGapMapUpdates, false);
  }
  ...
  for (const auto& invertedBlock : invertedBlocks) {
    ...
    gap_map::invertGapMap(gapMap, {beg, end}, gapMapBlocksBacktracks, dummyGapMapUpdates);
  }
