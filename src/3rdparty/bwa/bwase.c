...
{
	fprintf(stderr, "[samse_core] Starting samse_core with prefix: %s\n", prefix);
	...
	if (!bns) {
		fprintf(stderr, "[samse_core] Failed to restore bns with prefix: %s\n", prefix);
		return;
	}
	srand48(bns->seed);
	fp_sa = xopen(fn_sa, "r");
...
int bwa_sai2sam_se(int argc, char *argv[])
{
	...
	// Reset optind if getopt is used multiple times
	optind = 1;
	while ((c = getopt(argc, argv, "hn:f:r:")) != -1) {
		fprintf(stderr, "Looking at: %c\n", c);
		switch (c) {
		case 'h': 
			fprintf(stderr, "Debug: option -h encountered\n");
			break;
		case 'r':
			fprintf(stderr, "Debug: option -r with value %s\n", optarg);
			if ((rg_line = bwa_set_rg(optarg)) == 0) {
				fprintf(stderr, "Error: bwa_set_rg failed\n");
				return 1;
			}
			break;
		case 'n': 
			n_occ = atoi(optarg);
			fprintf(stderr, "Debug: option -n with value %d\n", n_occ);
			break;
		case 'f': 
			fprintf(stderr, "Debug: option -f with value %s\n", optarg);
	...
	fprintf(stderr, "optind after parsing: %d\n", optind);
	...
	// Print debug for args
	fprintf(stderr, "Debug: prefix = %s\n", prefix);
	fprintf(stderr, "Debug: in.sai = %s\n", argv[optind+1]);
	fprintf(stderr, "Debug: in.fq = %s\n", argv[optind+2]);
	fprintf(stderr, "Debug: n_occ = %d\n", n_occ);
	if (rg_line) {
		fprintf(stderr, "Debug: rg_line = %s\n", rg_line);
	} else {
		fprintf(stderr, "Debug: rg_line = NULL\n");
	}
