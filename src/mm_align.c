...
static char *emit_sam_se(const mm_idx_t *mi, const mm_mapopt_t *mopt,
                         int read_len, const char *read_seq,
                         const char *read_qual, const char *read_name,
                         mm_reg1_t *reg, int n_reg, int *out_rs) {
...
void align_reads(const char *reference, int n_reads, const char **reads,
  ...
  if (pairedEndReads) {
    for (int k = 0; k < n_reads / 2; k++) {
      ...
      if (n_reg[0] == 0 || n_reg[1] == 0 ||
          reg[0] == NULL || reg[1] == NULL ||
          reg[0]->score <= 0 || reg[1]->score <= 0) {
        ...
        mm_bseq1_t t1;
        t1.l_seq = qlens[0]; t1.seq = (char*)reads[k*2];
        t1.name = (char*)read_names[k*2]; t1.qual = (char*)quality[k*2];
        ...
        mm_bseq1_t t2;
        t2.l_seq = qlens[1]; t2.seq = (char*)reads[k*2+1];
        t2.name = (char*)read_names[k*2+1]; t2.qual = (char*)quality[k*2+1];
    ...
    for (int k = 0; k < n_reads; k++) {
      ...
      sam_alignments[k] = emit_sam_se(mi, &mopt, r_lens[k], reads[k],
                                       quality[k], read_names[k],
                                       reg, n_reg, &r_lens[k]);
      free_regs(reg, n_reg);
    }
  }
...
static void *align_worker_func(void *arg) {
  ...
  if (w->paired) {
    ...
    for (int k = w->start_pair; k < w->end_pair; k++) {
      int n_reg = 0;
      mm_reg1_t *reg = mm_map(w->mi, w->r_lens[k], w->reads[k],
                               &n_reg, tbuf, w->mopt, NULL);

      align_pair_result_t *res = &w->results[k];
      memset(res, 0, sizeof(*res));

      if (n_reg > 0 && reg && reg[0].score > 0 &&
          reg[0].score <= w->r_lens[k]) {
        res->mapped = 1;
...
void align_reads_direct(const char *reference, int n_reads,
  ...
  if (n_threads == 1) {
    ...
    for (int t = 0; t < n_threads; t++)
      pthread_create(&threads[t], NULL, align_worker_func, &workers[t]);
    for (int t = 0; t < n_threads; t++)
