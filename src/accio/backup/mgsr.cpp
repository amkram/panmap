...
void writeCurSeeds(const std::string& path, const std::map<int32_t, std::pair<size_t, int32_t>>& curSeeds, const tree::mutableTreeData& data) {
        std::ofstream ofSeeds(path);
        for (const auto& seed : curSeeds) {
            ofSeeds << seed.second.first << "\t"
                    << data.degap[seed.first] << "\t"
                    << data.degap[seed.second.second] << "\t"
                    << seed.first << "\t"
                    << seed.second.second << "\n";
        }
        ofSeeds.close();
...
//        std::cout << "blockMut: " << blockId << "  start: " << blockStart << "  stop: " << blockStop << "  newStrand: " << newStrand << std::endl;
    ...
    if (seedmersIndex.seedmerMap.size() == 0) {
        ...
        // std::cerr << node->identifier << "\tparent: " << node->parent->identifier << std::endl;
        if (true) {
            /*update seedmer and curSeeds*/
            std::cout << node->identifier << "\tparent: " << node->parent->identifier << std::endl;
            std::sort(syncmerChanges.begin(), syncmerChanges.end(), [](const auto &a, const auto &b) {
                return std::get<1>(a) < std::get<1>(b);
            });
        ...
        //     std::cout << curSeed.second.first << "\t"
        //               << curSeed.first << "\t"
        //               << curSeed.second.second << std::endl;
        // }
        std::string seedsPath = "../src/test/data/mgsr/test/" + node->identifier + ".smi";
        writeCurSeeds(seedsPath, curSeeds, data);
        // std::cout << "---------" << std::endl;
