...
#include "samtools_src/sam_utils.h"
#include "samtools_src/samtools.h"
#include "samtools_src/bedidx.h"
#include "samtools_src/sam_opts.h"
#include "samtools_src/bam_plbuf.h"
#include "samtools_src/sample.h"
...
static inline int printw(int c, kstring_t *k)
{
    char buf[16];
    int l, x;
    if (c == 0) {
...
int pileup_seq(kstring_t *mplp_string, const bam_pileup1_t *p, hts_pos_t pos,
               ...
               int no_del, int no_ends)
{
    ...
    if (!p->is_del) {
        ...
        kputc(p->is_refskip? (bam_is_rev(p->b)? '<' : '>') : ((bam_is_rev(p->b) && rev_del) ? '#' : '*'), mplp_string);
        //putc(p->is_refskip? (bam_is_rev(p->b)? '<' : '>') : ((bam_is_rev(p->b) && rev_del) ? '#' : '*'), fp);
    }
    int del_len = -p->indel;
    if (p->indel > 0) {
...
static void
print_empty_pileup(kstring_t *k,  const mplp_conf_t *conf, const char *tname,
                   hts_pos_t pos, int n, const char *ref, hts_pos_t ref_len)
{
    int i;
    ksprintf(k, "%s\t%"PRIhts_pos"\t%c", tname, pos+1, (ref && pos < ref_len)? ref[pos] : 'N');
 ...
 */
static int mpileup(mplp_conf_t *conf, sam_hdr_t *sam_header,  kstring_t *mplp_string)
{
            ...
            while (tid > last_tid) {
                if (last_tid >= 0 && !conf->reg) {
                    while (++last_pos < sam_hdr_tid2len(h, last_tid)) {
                        ...
                        print_empty_pileup(mplp_string, conf, sam_hdr_tid2name(h, last_tid), last_pos, n, ref, ref_len);
                    }
                }
                last_tid++;
                last_pos = -1;
        ...
        if (conf->all) {
            ...
            while (++last_pos < pos) {
                ...
                if (conf->bed && bed_overlap(conf->bed, sam_hdr_tid2name(h, tid), last_pos, last_pos + 1) == 0)
                    continue;
                //TODO handle empty case
                print_empty_pileup(mplp_string, conf, sam_hdr_tid2name(h, tid), last_pos, n, ref, ref_len);
            }
            last_tid = tid;
            last_pos = pos;
        }
        if (conf->bed && tid >= 0 && !bed_overlap(conf->bed, sam_hdr_tid2name(h, tid), pos, pos+1)) continue;
        ksprintf(mplp_string, "%s\t%"PRIhts_pos"\t%c", sam_hdr_tid2name(h, tid), pos + 1, (ref && pos < ref_len)? ref[pos] : 'N');
            ...
            if (n_plp[i] == 0) {
                ...
                for (j = 0; j < n_plp[i]; ++j) {
                    ...
                    if (c >= conf->min_baseQ) {
                        n++;
                        //TODO what do i do
                        if (pileup_seq( mplp_string, plp[i] + j, pos, ref_len,
                                       ref, &ks, conf->rev_del,
