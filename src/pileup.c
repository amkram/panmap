...
int our_pileup_seq(kstring_t *mplp_string, const bam_pileup1_t *p, hts_pos_t pos,
...
void our_print_empty_pileup(kstring_t *k,  const mplp_conf_t *conf, const char *tname,
            ...
            while (tid > last_tid) {
                if (last_tid >= 0 && !conf->reg) {
                    while (++last_pos < sam_hdr_tid2len(h, last_tid)) {
                        ...
                        our_print_empty_pileup(mplp_string, conf, sam_hdr_tid2name(h, last_tid), last_pos, n, ref, ref_len);
                    }
                }
                last_tid++;
                last_pos = -1;
        ...
        if (conf->all) {
            ...
            while (++last_pos < pos) {
                ...
                if (conf->bed && bed_overlap(conf->bed, sam_hdr_tid2name(h, tid), last_pos, last_pos + 1) == 0)
                    continue;
                //TODO handle empty case
                our_print_empty_pileup(mplp_string, conf, sam_hdr_tid2name(h, tid), last_pos, n, ref, ref_len);
            }
            last_tid = tid;
            last_pos = pos;
        }
            ...
            if (n_plp[i] == 0) {
                ...
                for (j = 0; j < n_plp[i]; ++j) {
                    ...
                    if (c >= conf->min_baseQ) {
                        n++;
                        //TODO what do i do
                        if (our_pileup_seq( mplp_string, plp[i] + j, pos, ref_len,
                                       ref, &ks, conf->rev_del,
