...
// r_poss: position of seeds(from both reads) in reference coords (the ends of the seed, the last index included in the seed)
// q_poss: position of seeds in query(read) coords (the ends of the seed, the last index included in the seed)
// reversed: whether seeds are reversed
void align_read_given_seeds(const mm_idx_t *mi, int num_reads, const int* read_lengths, const char **read_seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, int seed_l, int n_seeds_0, int n_seeds_1, int *r_poss, int *q_poss, uint8_t *reversed)
{
...
void align_reads(const char *reference, int n_reads, const char **reads, const char **quality, const char **read_names, int *r_lens, int *seed_counts, uint8_t **reversed, int **ref_positions, int **qry_positions, char** sam_alignments, int syncmer_k, bool pairedEndReads ) {
	...
	if (pairedEndReads) {
		...
		for(int k = 0; k < n_reads; k++) {
			...
			align_read_given_seeds(mi, 1, &(r_lens[k]), &(reads[k]), n_reg,  reg,  tbuf, &mopt, iopt.k, seed_counts[k], 0, ref_positions[k], qry_positions[k], reversed[k]);
