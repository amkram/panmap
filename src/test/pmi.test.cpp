...
void writeSeedmersFromIndexFile(seedmerIndex_t& index, std::unordered_map<int32_t, std::string> &seedmers, Tree *T, const Node *node, const std::string& testDirName);
...
BOOST_AUTO_TEST_CASE(seedmersIndex) {
    ...
    // std::vector<std::tuple<int, int, int>> parameters = {{10, 5, 2}, {10, 5, 1}, {15, 7, 3}, {15, 7, 1}};
    ...
    for (const auto& param : parameters) {
        ...
        std::string dirName = std::string("../src/test/data/pmi_test/true_seedmers") + "_k" + std::to_string(k) + "_s" + std::to_string(s) + "_l" + std::to_string(l) + "/";
        fs::create_directories(dirName);
        std::cerr << "Printing seedmers from sequence, k: " << k << ", s: " << s << ", l: " << l << " in " << dirName << std::endl;
        for (const auto& n : T->allNodes) {
            std::string node_idn = n.first;
            std::string outSeedmersPath = dirName + node_idn + ".pmi";
            if (fs::exists(outSeedmersPath)) continue;

            std::string node_seq = T->getStringFromReference(node_idn, false);
            std::vector<std::tuple<std::string, int, int>> seedmers = extractSeedmers(node_seq, k, s, l, false);
        
            std::ofstream osdmfs(outSeedmersPath);
            for (const auto& seedmer : seedmers) {
                osdmfs << std::get<0>(seedmer) << "\t"
                        << std::get<1>(seedmer) << "\t"
        ...
        std::string testDirName = std::string("../src/test/data/pmi_test/test_seedmers") + "_k" + std::to_string(k) + "_s" + std::to_string(s) + "_l" + std::to_string(l) + "/";
        std::cerr << "Building seedmers from index, k: " << k << ", s: " << s << ", l: " << l << " in " << dirName << std::endl;

        pmi::seedIndex index;
        pmi::build(index, T, l, k, s);
        std::string indexFile = std::string("../src/test/data/pmi_test/") + "k" + std::to_string(k) + "_s" + std::to_string(s) + "_l" + std::to_string(l) + ".pmi";
        ...
        std::cerr << "Printing seedmers from index, k: " << k << ", s: " << s << ", l: " << l << " in " << dirName << std::endl;

        std::unordered_map<int32_t, std::string> dynamicSeedmersTarget;
        writeSeedmersFromIndexFile(seedmerIndex, dynamicSeedmersTarget, T, T->root, testDirName);
    }

}

static void mutateSeedmerMap(std::unordered_map<int32_t, std::string> &seedmers, const std::string &nid, seedmerIndex_t &index, std::vector<std::pair<int32_t, std::string>> &seedmersToRevert) {
...
static void revertSeedmerMap(std::unordered_map<int32_t, std::string> &seedmers, const std::string &nid, seedmerIndex_t &index, std::vector<std::pair<int32_t, std::string>> &seedmersToRevert) {
...
void writeSeedmersFromIndexFile(seedmerIndex_t& index, std::unordered_map<int32_t, std::string> &seedmers, Tree *T, const Node *node, const std::string& testDirName) {
    ...
    for (const auto& s : seedmers) seedmersToWrite.emplace_back(std::make_pair(s.first, s.second));

    std::sort(seedmersToWrite.begin(), seedmersToWrite.end(), [](const auto& a, const auto& b) {
...
std::vector<std::tuple<std::string, int, int>> extractSeedmers(const std::string& seq, const int k, const int s, const int l, const bool open) {
    ...
    for (size_t i = 0; i < seq.size() - k + 1; ++i) {
        ...
        if (seeding::is_syncmer(kmer, s, open)) {
            syncmers.emplace_back(std::make_tuple(kmer, i, i + k -1));
        }
    }
    ...
    for (size_t i = 0; i < syncmers.size() - l + 1; ++i) {
        ...
        for (size_t j = 0; j < l; ++j) seedmerSeq += std::get<0>(syncmers[i+j]);
        seedmers.emplace_back(std::make_tuple(seedmerSeq, std::get<1>(syncmers[i]), std::get<2>(syncmers[i+l-1])));
    }
