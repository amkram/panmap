...
private:
    ...
    static inline std::unordered_map<std::thread::id, std::stack<std::pair<std::string, std::chrono::high_resolution_clock::time_point>>> timing_stacks{};
...
public:
    ...
    static void startBlock(const std::string& block_name, const std::string& function_name) {
    ...
    static void report() {
        ...
        for (const auto& [thread_id, stack] : timing_stacks) {
            if (!stack.empty()) {
                std::cerr << "Warning: Found " << stack.size() << " unclosed timer(s) in thread " 
                         << thread_id << "\n";
            }
        }
        ...
        // Sort by total time
        std::vector<std::pair<std::string, TimingStats>> sorted_stats(function_stats.begin(), function_stats.end());
        std::sort(sorted_stats.begin(), sorted_stats.end(),
                 [](const auto& a, const auto& b) { return a.second.total_time > b.second.total_time; });

        for (const auto& [func, stats] : sorted_stats) {
            if (stats.total_time < min_time_to_report) continue;

            double avg_time = stats.total_time / stats.calls;
            
            std::cout << std::left << std::setw(40) << func
                     << std::right << std::setw(10) << stats.calls
                     << std::right << std::setw(15) << formatTime(stats.total_time)
                     << std::right << std::setw(15) << formatTime(avg_time)
                     << std::right << std::setw(15) << formatTime(stats.min_time)
                     << std::right << std::setw(15) << formatTime(stats.max_time) << "\n";
        }
...
#define TIME_BLOCK(block_name) \
    Timer::startBlock(block_name, __func__); \
