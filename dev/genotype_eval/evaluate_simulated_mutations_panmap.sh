...
for ((i=0; i<NUM_REPLICATES; i++)); do
  ...
  for j in "${!NUM_READS_ARRAY[@]}"; do
    ...
    bcftools call --ploidy 1 -cv -Ov -o "${OUTPUT_DIR}/${OUTPUT_PREFIX}_bcf_vcf/${REF_STRAIN_MODIFIED}.variants.${i}.${COVERAGE}x.varonly.vcf" "${OUTPUT_DIR}/${OUTPUT_PREFIX}_bcf_vcf/${REF_STRAIN_MODIFIED}.variants.${i}.${COVERAGE}x.bcf" > /dev/null 2>&1
    ...
    panmap \
      ...
      "$READ_R2" \
      --genotype-from-sam \
      --sam-file "${OUTPUT_DIR}/${OUTPUT_PREFIX}_bam/${REF_STRAIN_MODIFIED}.${i}.${COVERAGE}x.sorted.sam" \
      --ref-file "$OUTPUT_DIR/${OUTPUT_PREFIX}_refFasta/${REF_STRAIN_MODIFIED}.fa" \
      --prefix "${OUTPUT_DIR}/${OUTPUT_PREFIX}_panmap_output/${REF_STRAIN_MODIFIED}.variants.${i}.${COVERAGE}x"
    ...
    echo "Creating roc files for panmap..."
    python3 filter_genotype_var.py \
      "${OUTPUT_DIR}/${OUTPUT_PREFIX}_panmap_output/${REF_STRAIN_MODIFIED}.variants.${i}.${COVERAGE}x.vcf" \
      "${OUTPUT_DIR}/${OUTPUT_PREFIX}_panmap_output/${REF_STRAIN_MODIFIED}.variants.${i}.${COVERAGE}x.filtered.vcf"
    vcfroc \
      -t "${OUTPUT_DIR}/${OUTPUT_PREFIX}_vcfTrue/${REF_STRAIN_MODIFIED}.var.${i}.vcf" \
      -r "${OUTPUT_DIR}/${OUTPUT_PREFIX}_refFasta/${REF_STRAIN_MODIFIED}.fa" \
      "${OUTPUT_DIR}/${OUTPUT_PREFIX}_panmap_output/${REF_STRAIN_MODIFIED}.variants.${i}.${COVERAGE}x.filtered.vcf" \
      > "${OUTPUT_DIR}/${OUTPUT_PREFIX}_roc_panmap/${REF_STRAIN_MODIFIED}.${i}.${COVERAGE}x.roc"
  done
done
